...
class AbstractWriterThreadCaseFlask(debugger_unittest.AbstractWriterThread):
    ...
    def write_add_breakpoint_jinja2(self, line, func, template):
        ...
        self.write(
            "111\t%s\t%s\t%s\t%s\t%s\t%s\tNone\tNone"
            % (self.next_seq(), breakpoint_id, "jinja2-line", template_file, line, func)
        )
        self.log.append(
            "write_add_breakpoint_jinja: %s line: %s func: %s"
    ...
    def write_add_exception_breakpoint_jinja2(self, exception="jinja2-Exception"):
        self.write(
            "%s\t%s\t%s\t%s\t%s\t%s"
            % (CMD_ADD_EXCEPTION_BREAK, self.next_seq(), exception, 2, 0, 0)
        )
...
class AbstractWriterThreadCaseDjango(debugger_unittest.AbstractWriterThread):
    ...
    def write_add_breakpoint_django(self, line, func, template):
        ...
        self.write(
            "111\t%s\t%s\t%s\t%s\t%s\t%s\tNone\tNone"
            % (self.next_seq(), breakpoint_id, "django-line", template_file, line, func)
        )
        self.log.append(
            "write_add_django_breakpoint: %s line: %s func: %s"
            % (breakpoint_id, line, func)
        )
        return breakpoint_id
    ...
    def write_add_exception_breakpoint_django(self, exception="Exception"):
        self.write(
            "%s\t%s\t%s" % (CMD_ADD_DJANGO_EXCEPTION_BREAK, self.next_seq(), exception)
        )

    def write_remove_exception_breakpoint_django(self, exception="Exception"):
        self.write(
            "%s\t%s\t%s"
            % (CMD_REMOVE_DJANGO_EXCEPTION_BREAK, self.next_seq(), exception)
        )
        ...
        def test_file(
            ...
            if self.check_non_ascii:
                ...
                new_filename = os.path.join(
                    new_dir, self.NON_ASCII_CHARS + os.path.basename(filename)
                )
    ...
    def additional_output_checks(writer, stdout, stderr):
        ...
        if "ValueError: TEST SUCEEDED" not in stderr:
            raise AssertionError(
                '"ValueError: TEST SUCEEDED" not in stderr.\nstdout:\n%s\n\nstderr:\n%s'
                % (stdout, stderr)
            )
        ...
        def test_file(
            ...
            **kwargs
            ...
            with debugger_runner_remote.check_case(
                WriterThread, wait_for_port=wait_for_port
            ) as writer:
                yield writer
...
def case_setup_remote_attach_to_dap(debugger_runner_remote):
    ...
    class CaseSetup(object):
        ...
        def test_file(self, filename, port, **kwargs):
            ...
            with debugger_runner_remote.check_case(
                WriterThread, wait_for_port=False
            ) as writer:
                yield writer
    ...
    class CaseSetup(object):
        ...
        def test_file(self, **kwargs):
            ...
            if version == [1, 7]:
                ...
                raise AssertionError(
                    "Can only check django 1.7 -> 4.1 right now. Found: %s" % (version,)
                )
            ...
            with debugger_runner_simple.check_case(
                WriterThread, dap=self.dap
            ) as writer:
                yield writer
