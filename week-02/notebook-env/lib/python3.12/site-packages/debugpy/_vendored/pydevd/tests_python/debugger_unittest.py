...
CMD_THREAD_DUMP_TO_STDERR = (
    153  # This is mostly for unit-tests to diagnose errors on ci.
)
...
CMD_GET_SMART_STEP_INTO_VARIANTS = (
    163  # XXX: PyCharm has 160 for this (we're currently incompatible anyways).
)
...
class DebuggerRunner(object):
    def __init__(self, tmpdir):
        if tmpdir is not None:
            self.pydevd_debug_file = os.path.join(
                str(tmpdir), "pydevd_debug_file_%s.txt" % (os.getpid(),)
            )
        else:
            ...
            try:
                ...
                with self.run_process(args, writer) as dct_with_stdout_stder:
                    try:
                        ...
                        writer.get_stdout = lambda: "".join(
                            dct_with_stdout_stder["stdout"]
                        )
                        writer.get_stderr = lambda: "".join(
                            dct_with_stdout_stder["stderr"]
                        )
    ...
    def run_process(self, args, writer):
        ...
        try:
            start_in_daemon_thread(
                read_process, (process.stdout, stdout, sys.stdout, "stdout", finish)
            )
            start_in_daemon_thread(
                read_process, (process.stderr, stderr, sys.stderr, "stderr", finish)
            )
                ...
                if process.poll() is not None:
                    if writer.EXPECTED_RETURNCODE != "any":
                        ...
                        if process.returncode not in expected_returncode:
                            self.fail_with_message(
                                "Expected process.returncode to be %s. Found: %s"
                                % (writer.EXPECTED_RETURNCODE, process.returncode),
                                stdout,
                                stderr,
                                writer,
                            )
                    ...
                    if writer is not None:
                        ...
                        if not shown_intermediate and (
                            time.time() - initial_time > (TIMEOUT / 3.0)
                        ):  # 1/3 of timeout
                            print(
            ...
            if writer is not None:
                if not writer.FORCE_KILL_PROCESS_WHEN_FINISHED_OK:
                    ...
                    while not writer.check_test_suceeded_msg(stdout, stderr):
                        ...
                        if check == 50:
                            self.fail_with_message(
                                "TEST SUCEEDED not found.", stdout, stderr, writer
                            )
                        time.sleep(0.1)
...
class AbstractWriterThread(threading.Thread):
    ...
    def start_socket_client(self, host, port):
        ...
        for _i in range(20):
            try:
                ...
                time.sleep(
                    0.5
                )  # We may have to wait a bit more and retry (especially on PyPy).
    ...
    def wait_for_single_notification_as_hit(
    ...
    def wait_for_breakpoint_hit(
        self, reason=REASON_STOP_ON_BREAKPOINT, timeout=None, **kwargs
    ):
        """
        108 is over
        109 is return
    ...
    def _get_stack_as_hit(self, msg, file=None, line=None, name=None):
        ...
        if file is not None:
            assert frame_file.endswith(
                file
            ), "Expected hit to be in file %s, was: %s" % (file, frame_file)

        if line is not None:
            assert (
                line == frame_line
            ), "Expected hit to be in line %s, was: %s (in file: %s)" % (
                line,
                frame_line,
                frame_file,
            )
    ...
    def write_version(self):
        ...
        self.write(
            "%s\t%s\t1.0\t%s\tID"
            % (CMD_VERSION, self.next_seq(), "WINDOWS" if IS_WINDOWS else "UNIX")
        )
    ...
    def write_show_return_vars(self, show=1):
        self.write(
            "%s\t%s\tCMD_SHOW_RETURN_VALUES\t%s"
            % (CMD_SHOW_RETURN_VALUES, self.next_seq(), show)
        )
    ...
    def write_add_breakpoint(
        ...
        if (
            hit_condition is None
            and not is_logpoint
            and suspend_policy is None
        ...
        self.log.append(
            "write_add_breakpoint: %s line: %s func: %s" % (breakpoint_id, line, func)
        )
        return breakpoint_id

    def write_multi_threads_single_notification(
    ...
    def write_suspend_on_breakpoint_exception(
        self, skip_suspend_on_breakpoint_exception, skip_print_breakpoint_exception
    ):
        self.write_json_config(
            dict(
    ...
    def write_json_config(self, config_dict):
        self.write(
            "%s\t%s\t%s"
            % (CMD_PYDEVD_JSON_CONFIG, self.next_seq(), json.dumps(config_dict))
        )
    ...
    def write_get_current_exception(self, thread_id):
        self.write(
            "%s\t%s\t%s" % (CMD_GET_EXCEPTION_DETAILS, self.next_seq(), thread_id)
        )
        ...
        self.write(
            ...
            % (
                ...
                % (
                    ...
                    "true"
                    if ignore_exceptions_thrown_in_lines_with_ignore_exception
                    else "false",
    ...
    def write_start_redirect(self):
        self.write(
            "%s\t%s\t%s" % (CMD_REDIRECT_OUTPUT, self.next_seq(), "STDERR STDOUT")
        )
    ...
    def write_set_project_roots(self, project_roots):
        self.write(
            ...
            % (
                ...
                self.next_seq(),
                "\t".join(str(x) for x in project_roots),
            )
        )
    ...
    def write_remove_exception_breakpoint(self, exception):
        self.write(
            "%s\t%s\t%s" % (CMD_REMOVE_EXCEPTION_BREAK, self.next_seq(), exception)
        )
    ...
    def write_get_frame(self, thread_id, frame_id):
        self.write(
            "%s\t%s\t%s\t%s\tFRAME"
            % (CMD_GET_FRAME, self.next_seq(), thread_id, frame_id)
        )
    ...
    def write_get_variable(self, thread_id, frame_id, var_attrs):
        self.write(
            "%s\t%s\t%s\t%s\tFRAME\t%s"
            % (CMD_GET_VARIABLE, self.next_seq(), thread_id, frame_id, var_attrs)
        )
    ...
    def write_debug_console_expression(self, locator):
        self.write(
            "%s\t%s\t%s" % (CMD_EVALUATE_CONSOLE_EXPRESSION, self.next_seq(), locator)
        )
    ...
    def write_evaluate_expression(self, locator, expression):
        self.write(
            "%s\t%s\t%s\t%s\t1"
            % (CMD_EVALUATE_EXPRESSION, self.next_seq(), locator, expression)
        )
    ...
    def wait_for_json_message(self, accept_message, unquote_msg=True, timeout=None):
        last = self.wait_for_message(
            accept_message, unquote_msg, expect_xml=False, timeout=timeout
        )
    ...
    def wait_for_untangled_message(
        self, accept_message, timeout=None, double_unquote=False
    ):
        import untangle
        from io import StringIO
        ...
        class T(threading.Thread):
            def wait_for_contents(self):
                for _ in range(10):
                    ...
                    raise AssertionError(
                        "Unable to get contents from server. Url: %s" % (full_url,)
                    )
