...
def test_convert_utilities(tmpdir):
    ...
    if IS_WINDOWS:
        ...
        for i in range(3):  # Check if cache is ok.
            ...
            assert (
                pydevd_file_utils.get_path_with_real_case("<does not EXIST>")
                == "<does not EXIST>"
            )
            ...
            if i == 2:
                ...
                assert pydevd_file_utils._listdir_cache[
                    os.path.dirname(normalized).lower()
                ] == ["Test_Convert_Utilities"]
                assert (
                    pydevd_file_utils._listdir_cache[
                        (
                            os.path.dirname(normalized).lower(),
                            "Test_Convert_Utilities".lower(),
                        )
                    ]
                    == real_case
                )
...
def test_source_reference(tmpdir):
    ...
    if IS_WINDOWS:
        ...
        assert pydevd_file_utils.map_file_to_client("c:\\bar\\my") == (
            "c:\\foo\\my",
            True,
        )
        assert (
            pydevd_file_utils.get_client_filename_source_reference("c:\\foo\\my") == 0
        )

        assert pydevd_file_utils.map_file_to_client("c:\\another\\my") == (
            "c:\\another\\my",
            False,
        )
        ...
        assert pydevd_file_utils.map_file_to_client("/another/my") == (
            "\\another\\my",
            False,
        )
        source_reference = pydevd_file_utils.get_client_filename_source_reference(
    ...
    try:
        ...
        if IS_WINDOWS:
            ...
            for in_eclipse, in_python in [
                ...
                check(
                    pydevd_file_utils.map_file_to_server("c:\\foo\\my"), "c:\\bar\\my"
                )
                check(pydevd_file_utils.map_file_to_server("c:/foo/my"), "c:\\bar\\my")
                check(pydevd_file_utils.map_file_to_server("c:/foo/my/"), "c:\\bar\\my")
                check(
                    pydevd_file_utils.map_file_to_server("c:\\foo\\áéíóú".upper()),
                    "c:\\bar" + "\\áéíóú".upper(),
                )
                check(
                    pydevd_file_utils.map_file_to_client("c:\\bar\\my"),
                    ("c:\\foo\\my", True),
                )
            ...
            for in_eclipse, in_python in [
                ...
                check(
                    pydevd_file_utils.map_file_to_client("c:/bar/my"), ("/foo/my", True)
                )
                check(
                    pydevd_file_utils.map_file_to_client("c:/bar/my/"),
                    ("/foo/my", True),
            ...
            assert (
                pydevd_file_utils.map_file_to_server(
                    os.path.join(in_eclipse, "another")
                )
                == os.path.join(in_python, "another").lower()
            )
            ...
            pydevd_file_utils.setup_client_server_paths(PATHS_FROM_ECLIPSE_TO_PYTHON)
            assert (
                pydevd_file_utils.map_file_to_server("/foo").lower()
                == in_python.lower()
            )
            ...
            assert pydevd_file_utils.map_file_to_client(test_dir.lower())[0].endswith(
                "\\Foo"
            )
        else:
            # Client on windows and server on unix
            ...
            for in_eclipse, in_python in [
                ...
                assert (
                    pydevd_file_utils.map_file_to_server("c:\\foo\\my") == "/báéíóúr/my"
                )
                assert (
                    pydevd_file_utils.map_file_to_server("C:\\foo\\my") == "/báéíóúr/my"
                )
                ...
                assert (
                    pydevd_file_utils.map_file_to_server("C:\\foo\\MY\\")
                    == "/báéíóúr/MY"
                )
                assert (
                    pydevd_file_utils.map_file_to_server("c:\\foo\\my\\file.py")
                    == "/báéíóúr/my/file.py"
                )
                assert (
                    pydevd_file_utils.map_file_to_server("c:\\foo\\my\\other\\file.py")
                ...
                assert pydevd_file_utils.map_file_to_client("/usr/bin/x.py") == (
                    "\\usr\\bin\\x.py",
                    False,
                )
                ...
                assert (
                    pydevd_file_utils.map_file_to_server("\\usr\\bin\\") == "/usr/bin"
                )
                ...
                assert (
                    pydevd_file_utils.map_file_to_server("c:\\Another") == "c:/Another"
                )

                assert (
                    pydevd_file_utils.map_file_to_server("c:/FoO/my/BAR")
                    == "/báéíóúr/my/BAR"
                )
                assert pydevd_file_utils.map_file_to_client("/báéíóúr/my/BAR") == (
                    "c:\\foo\\my\\BAR",
                    True,
                )
...
def test_zip_paths(tmpdir):
    ...
    for i, zip_basename in enumerate(("MY1.zip", "my2.egg!")):
        ...
        (
            ...
            realpath,
            basename,
        ) = pydevd_file_utils.get_abs_path_real_path_and_base_from_file(zipfile_path)
        ...
        for path in [
            ...
            assert pydevd_file_utils.exists(
                path
            ), "Expected exists to return True for path:\n%s" % (path,)
            (
                abspath,
                realpath,
                basename,
            ) = pydevd_file_utils.get_abs_path_real_path_and_base_from_file(path)
            assert pydevd_file_utils.exists(
                abspath
            ), "Expected exists to return True for path:\n%s" % (abspath,)
            assert pydevd_file_utils.exists(
...
def test_source_mapping():
    ...
    mapping = [
        SourceMappingEntry(
            line=3, end_line=6, runtime_line=5, runtime_source="<cell1>"
        ),
        SourceMappingEntry(
            line=10, end_line=11, runtime_line=1, runtime_source="<cell2>"
        ),
    ]
...
def test_mapping_conflict_to_server():
    ...
    assert (
        pydevd_file_utils.map_file_to_server("/opt/pathsomething/foo.py")
        == "/var/home/p2/foo.py"
    )

    assert (
        pydevd_file_utils.map_file_to_server("/opt/v2/pathsomething/foo.py")
        == "/var/home/p4/foo.py"
    )
    ...
    # so, it's possible that this is what the user actually wants.
    assert (
        pydevd_file_utils.map_file_to_server("/opt/v2/path_r1/foo.py")
        == "/var/home/p3_r1/foo.py"
    )
    ...
    # match it with the slash in the end.
    assert (
        pydevd_file_utils.map_file_to_server("/opt/pathsomething_foo.py")
        == "/opt/pathsomething_foo.py"
    )
