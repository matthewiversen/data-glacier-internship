                ...
                if FORCE_PYDEVD_VC_VARS:
                    env.update(
                        get_environment_from_batch_command(
                            [FORCE_PYDEVD_VC_VARS], initial=os.environ.copy()
                        )
                    )
                else:
                    ...
                    if is_python_64bit():
                        env.update(
                            get_environment_from_batch_command(
                                [vcvarsall, "amd64"], initial=os.environ.copy()
                            )
                        )
                    else:
                        env.update(
                            get_environment_from_batch_command(
                                [vcvarsall, "x86"], initial=os.environ.copy()
                            )
                        )

            else:
                raise AssertionError(
                    "Unable to setup environment for Python: %s" % (sys.version,)
                )
