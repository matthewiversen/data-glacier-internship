...
def test_find_thread():
    ...
    assert (
        pydevd_find_thread_by_id(get_current_thread_id(threading.current_thread()))
...
def test_convert_dap_log_message_to_expression():
    ...
    assert check_dap_log_message(
        "a {{22:22}} {2}", "'a %s %s' % ({22:22}, 2,)", "a {22: 22} 2"
    )
    assert check_dap_log_message(
        "a {(22,33)}} {2}", "'a %s} %s' % ((22,33), 2,)", "a (22, 33)} 2"
    )
...
def _check_tracing_other_threads():
    ...
    wait_for_condition(
        lambda: len(threads) == 2, msg=lambda: "Found threads: %s" % (threads,)
    )
...
def _build_launch_env():
    ...
    resources_dir = os.path.join(
        os.path.dirname(pydevd.__file__), "tests_python", "resources"
    )
...
def _check_in_separate_process(
...
@pytest.mark.skipif(
    not IS_CPYTHON, reason="Functionality to trace other threads requires CPython."
)
...
def test_find_main_thread_id():
    ...
    _check_in_separate_process(
        "check_main_thread_id_simple", "_pydevd_test_find_main_thread_id"
    )
    _check_in_separate_process(
        "check_main_thread_id_multiple_threads", "_pydevd_test_find_main_thread_id"
    )
    _check_in_separate_process("check_win_threads", "_pydevd_test_find_main_thread_id")
    _check_in_separate_process(
        "check_fix_main_thread_id_multiple_threads", "_pydevd_test_find_main_thread_id"
    )
    ...
    subprocess.check_call(
        [sys.executable, "-m", "_pydevd_test_find_main_thread_id"], env=environ, cwd=cwd
    )
    ...
    subprocess.check_call(
        [
            sys.executable,
            os.path.join(resources_dir, "_pydevd_test_find_main_thread_id.py"),
        ],
        env=environ,
        cwd=cwd,
    )
...
def test_gevent_notify():
    _check_in_separate_process(
        "check_notify_on_gevent_loaded", update_env={"GEVENT_SUPPORT": ""}
    )
    _check_in_separate_process(
        "check_dont_notify_on_gevent_loaded", update_env={"GEVENT_SUPPORT": "True"}
    )


@pytest.mark.skipif(
    True,
    reason="Skipping because running this test can interrupt the test suite execution.",
)
