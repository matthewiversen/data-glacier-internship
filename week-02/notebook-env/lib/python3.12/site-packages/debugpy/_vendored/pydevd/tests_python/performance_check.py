...
CHECK_BASELINE, CHECK_REGULAR, CHECK_CYTHON, CHECK_FRAME_EVAL = (
    "baseline",
    "regular",
    "cython",
    "frame_eval",
    ...
    def get_pydevd_file(self):
        if self.CHECK == CHECK_BASELINE:
            return os.path.abspath(
                os.path.join(r"X:\PyDev.Debugger.baseline", "pydevd.py")
            )
...
class CheckDebuggerPerformance(debugger_unittest.DebuggerRunner):
    ...
    def obtain_results(self, benchmark_name, filename):
        ...
        for _ in range(runs):
            ...
            print(
                "partial for: %s: %.3fs"
                % (writer_thread_class.BENCHMARK_NAME, all_times[-1])
            )
        if len(all_times) > 3:
        ...
        if "SPEEDTIN_AUTHORIZATION_KEY" in os.environ:
            ...
            for project_id in project_ids:
                api = pyspeedtin.PySpeedTinApi(
                    authorization_key=SPEEDTIN_AUTHORIZATION_KEY, project_id=project_id
                )
                ...
                (
                    ...
                    branch,
                    commit_date,
                ) = api.git_commit_id_branch_and_date_from_path(pydevd.__file__)
        ...
        self.performance_msg = "%s: %.3fs " % (
            writer_thread_class.BENCHMARK_NAME,
            time_when_debugged,
        )

    def method_calls_with_breakpoint(self):
        for writer in self.obtain_results(
            "method_calls_with_breakpoint", "_performance_1.py"
        ):
    ...
    def method_calls_without_breakpoint(self):
        for writer in self.obtain_results(
            "method_calls_without_breakpoint", "_performance_1.py"
        ):
            writer.write_make_initial_run()
            writer.finished_ok = True
    ...
    def method_calls_with_step_over(self):
        for writer in self.obtain_results(
            "method_calls_with_step_over", "_performance_1.py"
        ):
    ...
    def method_calls_with_exception_breakpoint(self):
        for writer in self.obtain_results(
            "method_calls_with_exception_breakpoint", "_performance_1.py"
        ):
    ...
    def global_scope_1_with_breakpoint(self):
        for writer in self.obtain_results(
            "global_scope_1_with_breakpoint", "_performance_2.py"
        ):
            writer.write_add_breakpoint(
                writer.get_line_index_with_content("Breakpoint here"), None
            )
            writer.write_make_initial_run()
            writer.finished_ok = True
    ...
    def global_scope_2_with_breakpoint(self):
        for writer in self.obtain_results(
            "global_scope_2_with_breakpoint", "_performance_3.py"
        ):
