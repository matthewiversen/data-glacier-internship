    ...
    def format_completion_message(self, defFile, completionsList):
        ...
        for tup in completionsList:
            ...
            if len(tup) > 2:
                compMsg.append(",")
                compMsg.append(
                    self.remove_invalid_chars(tup[2])
                )  # args - only if function.
...
class CompletionServer:
    ...
    def connect_to_server(self):
        ...
        try:
            ...
            sys.stderr.write(
                "Error on connect_to_server with parameters: host: %s port: %s\n"
                % (HOST, self.port)
            )
            raise
    ...
    def run(self):
        ...
        try:
            ...
            dbg(
                SERVER_NAME
                + " connecting to java server on %s (%s)" % (HOST, self.port),
                INFO1,
            )
            ...
            while not self.ended:
                ...
                try:
                    try:
                        ...
                        if data.find(MSG_PYTHONPATH) != -1:
                            ...
                            if data.startswith(MSG_IMPORTS):
                                ...
                                try:
                                    ...
                                    self.send(
                                        self.get_completions_message("empty", lst)
                                    )
                        ...
                        e = sys.exc_info()[1]
                        msg = self.get_completions_message(
