...
pytestmark = pytest.mark.skipif(
    IS_JYTHON, reason="Single notification is not OK in Jython (investigate)."
)
...
class JsonFacade(object):
    ...
    def wait_for_json_message(self, expected_class, accept_message=lambda obj: True):
        ...
        msg = self.writer.wait_for_message(
            accept_json_message, unquote_msg=False, expect_xml=False
        )
    ...
    def write_make_initial_run(self):
        ...
        configuration_done_request = self.write_request(
            pydevd_schema.ConfigurationDoneRequest()
        )
        return self.wait_for_response(configuration_done_request)

    def write_list_threads(self):
        return self.wait_for_response(
            self.write_request(pydevd_schema.ThreadsRequest())
        )
    ...
    def wait_for_thread_stopped(
            ...
            if not path.endswith(file):
                ...
                if not path.endswith(file):
                    raise AssertionError(
                        "Expected path: %s to end with: %s" % (path, file)
                    )
        if name is not None:
        ...
        if line is not None:
            ...
            assert (
                found_line in line
            ), "Expect to break at line: %s. Found: %s (file: %s)" % (
                line,
                found_line,
                path,
            )
    ...
    def write_set_breakpoints(
        ...
        if isinstance(filename, bytes):
            ...
            )  # file is in the filesystem encoding but protocol needs it in utf-8
        ...
        for line in lines:
            ...
            breakpoints.append(
                pydevd_schema.SourceBreakpoint(
                    line,
                    condition=condition,
    ...
    def write_set_exception_breakpoints(self, filters=None, exception_options=None):
        ...
        exception_options = [
            exception_option.to_dict() for exception_option in exception_options
        ]

        arguments = pydevd_schema.SetExceptionBreakpointsArguments(
            filters=filters, exceptionOptions=exception_options
        )
    ...
    def write_disconnect(self, wait_for_response=True, terminate_debugee=False):
        ...
        self._sent_launch_or_attach = False
        arguments = pydevd_schema.DisconnectArguments(
            terminateDebuggee=terminate_debugee
        )
    ...
    def get_stack_as_json_hit(self, thread_id, no_stack_frame=False):
        stack_trace_request = self.write_request(
            pydevd_schema.StackTraceRequest(
                pydevd_schema.StackTraceArguments(threadId=thread_id)
            )
        )
    ...
    def get_variables_response(self, variables_reference, fmt=None, success=True):
        ...
        variables_request = self.write_request(
            pydevd_schema.VariablesRequest(
                pydevd_schema.VariablesArguments(variables_reference, format=fmt)
            )
        )
    ...
    def write_step_in(self, thread_id, target_id=None):
        arguments = pydevd_schema.StepInArguments(
            threadId=thread_id, targetId=target_id
        )
        self.wait_for_response(
            self.write_request(pydevd_schema.StepInRequest(arguments))
        )
    ...
    def write_set_variable(self, frame_variables_reference, name, value, success=True):
        ...
        if set_variable_response.success != success:
            raise AssertionError(
                ...
                % (
                    success,
                    set_variable_response.success,
                    set_variable_response.to_json(),
                )
            )
        return set_variable_response

    def get_name_to_scope(self, frame_id):
        scopes_request = self.write_request(
            pydevd_schema.ScopesRequest(pydevd_schema.ScopesArguments(frame_id))
        )
        ...
        name_to_scopes = dict(
            (scope["name"], pydevd_schema.Scope(**scope)) for scope in scopes
        )
    ...
    def get_step_in_targets(self, frame_id):
        request = self.write_request(
            pydevd_schema.StepInTargetsRequest(
    ...
    def get_name_to_var(self, variables_reference):
        ...
        return dict(
            (variable["name"], pydevd_schema.Variable(**variable))
    ...
    def write_initialize(self, success=True):
        ...
        response = self.wait_for_response(
            self.write_request(InitializeRequest(arguments))
        )
        assert response.success == success
        if success:
    ...
    def write_authorize(self, access_token, success=True):
        ...
        response = self.wait_for_response(
            self.write_request(PydevdAuthorizeRequest(arguments))
        )
        assert response.success == success
        return response
    ...
    def evaluate(
        ...
        eval_request = self.write_request(
            pydevd_schema.EvaluateRequest(
                pydevd_schema.EvaluateArguments(
    ...
    def write_get_source(self, source_reference, success=True):
        response = self.wait_for_response(
            self.write_request(
                pydevd_schema.SourceRequest(
                    pydevd_schema.SourceArguments(source_reference)
                )
            )
        )
        assert response.success == success
    ...
    with case_setup_dap.test_file("_debugger_case_change_breaks.py") as writer:
        ...
        if scenario == "basic":
            json_facade.write_set_breakpoints(
                [break_2, break_3],
                line_to_info={break_2: {"log_message": 'var {repr("_a")} is {_a}'}},
            )
...
def test_case_json_logpoint_and_step_failure_ok(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_hit_count.py") as writer:
        ...
        json_facade.write_set_breakpoints(
            ...
            line_to_info={print_line: {"log_message": 'var {repr("_a")} is {_a}'}},
        )
        json_facade.write_make_initial_run()
...
def test_case_json_logpoint_and_step_still_prints(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_hit_count.py") as writer:
        ...
        json_facade.write_set_breakpoints(
            [before_loop_line, print_line],
            line_to_info={print_line: {"log_message": 'var {repr("i")} is {i}'}},
        )
        json_facade.write_make_initial_run()
...
def test_case_json_hit_count_and_step(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_hit_count.py") as writer:
        ...
        print_line = writer.get_line_index_with_content("print line")
        json_facade.write_set_breakpoints(
            [print_line], line_to_info={print_line: {"hit_condition": "5"}}
        )
        json_facade.write_make_initial_run()

        json_hit = json_facade.wait_for_thread_stopped(line=print_line)
        i_local_var = json_facade.get_local_var(
            json_hit.frame_id, "i"
        )  # : :type i_local_var: pydevd_schema.Variable
...
def test_case_json_hit_condition_error(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_hit_count.py") as writer:
        ...
        json_facade.write_set_breakpoints(
            [bp], line_to_info={bp: {"condition": "range.range.range"}}
        )
        json_facade.write_make_initial_run()
        ...
        def accept_message(msg):
            if msg.body.category == "important":
                if (
                    "Error while evaluating expression in conditional breakpoint"
                    in msg.body.output
                ):
                    return True
            return False
...
def test_case_json_change_breaks(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_change_breaks.py") as writer:
        ...
        json_facade.write_set_breakpoints(
            break1_line, success=False, send_launch_if_needed=False
        )
...
def test_case_throw_exc_reason(case_setup_dap):
    ...
    with case_setup_dap.test_file(
        ...
        json_hit = json_facade.wait_for_thread_stopped(
            reason="exception",
            line=writer.get_line_index_with_content("raise RuntimeError from e"),
        )

        exc_info_request = json_facade.write_request(
            pydevd_schema.ExceptionInfoRequest(
                pydevd_schema.ExceptionInfoArguments(json_hit.thread_id)
            )
        )
        ...
        lines_and_names = re.findall(
            r",\sline\s(\d+),\sin\s(\[Chained Exception\]\s)?([\w|<|>]+)",
...
def test_case_throw_exc_reason_shown(case_setup_dap):
    ...
    with case_setup_dap.test_file(
        ...
        json_hit = json_facade.wait_for_thread_stopped(
            reason="exception",
            line=writer.get_line_index_with_content(
...
def test_case_handled_exception_breaks(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_exceptions.py") as writer:
        ...
        json_facade.wait_for_thread_stopped(
            reason="exception",
            line=writer.get_line_index_with_content("reraise on method2"),
        )
    ...
    for frame in json_hit.stack_trace_response.body.stackFrames:
        if "(Current frame)" in frame["name"]:
            if frame["line"] not in current_line:
                rep = json.dumps(
                    json_hit.stack_trace_response.body.stackFrames, indent=4
                )
                raise AssertionError(
                    "Expected: %s to be one of: %s\nFrames:\n%s."
                    % (frame["line"], current_line, rep)
                )
        ...
        raise AssertionError(
            "Could not find (Current frame) in any frame name in: %s." % (rep)
        )
...
def test_case_user_unhandled_exception(case_setup_dap, stop):
    def get_environ(self):
        ...
        env["IDE_PROJECT_ROOTS"] = (
            os.path.dirname(self.TEST_FILE) + os.pathsep + os.path.abspath(".")
        )
        return env
    ...
    with case_setup_dap.test_file(target, get_environ=get_environ) as writer:
        ...
        if stop:
            json_hit = json_facade.wait_for_thread_stopped(
                reason="exception",
                line=writer.get_line_index_with_content("raise here"),
                file=target,
            )
            _check_current_line(
                json_hit, writer.get_line_index_with_content("stop here")
            )
...
def test_case_unhandled_exception_just_my_code(case_setup_dap, target, just_my_code):
    ...
    def additional_output_checks(writer, stdout, stderr):
        if "call_exception_in_exec()" not in stderr:
            raise AssertionError(
                "Expected test to have an unhandled exception.\nstdout:\n%s\n\nstderr:\n%s"
                % (stdout, stderr)
            )
...
@pytest.mark.skipif(
    not IS_PY36_OR_GREATER, reason="Python 3.6 onwards required for test."
)
...
def test_case_unhandled_exception(case_setup_dap, target_file):
    ...
    with case_setup_dap.test_file(
        ...
        json_facade.wait_for_thread_stopped(
            reason="exception",
            line=(line_in_thread1, line_in_thread2),
            file=target_file,
        )
        json_facade.write_continue()
...
def test_case_sys_exit_unhandled_exception(case_setup_dap):
    with case_setup_dap.test_file(
        "_debugger_case_sysexit.py", EXPECTED_RETURNCODE=1
    ) as writer:
        json_facade = JsonFacade(writer)
        json_facade.write_set_exception_breakpoints(["uncaught"])
        json_facade.write_make_initial_run()
...
@pytest.mark.parametrize(
    "target", ["_debugger_case_sysexit_0.py", "_debugger_case_sysexit_none.py"]
)
def test_case_sys_exit_0_unhandled_exception(
    case_setup_dap, break_on_system_exit_zero, target
):
...
def test_case_sys_exit_0_handled_exception(case_setup_dap, break_on_system_exit_zero):
    with case_setup_dap.test_file(
        "_debugger_case_sysexit_0.py", EXPECTED_RETURNCODE=0
    ) as writer:
        json_facade = JsonFacade(writer)
        json_facade.write_launch(
...
def test_case_started_exited_threads_protocol(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_thread_started_exited.py") as writer:
        ...
        started_events = json_facade.mark_messages(
            ThreadEvent, lambda x: x.body.reason == "started"
        )
        exited_events = json_facade.mark_messages(
            ThreadEvent, lambda x: x.body.reason == "exited"
        )
...
def test_case_exclude_double_step(case_setup_dap):
    with case_setup_dap.test_file("my_code/my_code_double_step.py") as writer:
        ...
        json_facade.write_step_in(json_hit.thread_id)
        json_hit = json_facade.wait_for_thread_stopped(
            "step", file="my_code_double_step.py", line=break_line + 1
        )
    ...
    with case_setup_dap.test_file("my_code/my_code.py") as writer:
        ...
        if custom_setup == "set_exclude_launch_path_match_filename":
            ...
            response = json_facade.write_set_breakpoints(
                1, filename=other_filename, verified=False
            )
            ...
            other_filename = os.path.join(
                not_my_code_dir, "file_that_does_not_exist.py"
            )
            response = json_facade.write_set_breakpoints(
                1, filename=other_filename, verified=False
            )
        ...
        messages = json_facade.mark_messages(
            OutputEvent,
            lambda output_event: "Frame skipped from debugging during step-in."
            in output_event.body.output,
        )
        assert len(messages) == 1
    ...
    with case_setup_dap.test_file("_debugger_case_completions.py") as writer:
        ...
        for i in range(2):
            ...
            assert set(labels).issuperset(
                set(["__contains__", "items", "keys", "values"])
            )
...
def test_modules(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_local_variables.py") as writer:
        ...
        response = json_facade.wait_for_response(
            json_facade.write_request(
...
def test_dict_ordered(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_odict.py") as writer:
        ...
        assert [
            ...
            if (not d["name"].startswith("_OrderedDict"))
            and (d["name"] not in DAPGrouper.SCOPES_SORTED)
        ] == [
            ("4", "'first'"),
            ("3", "'second'"),
            ("2", "'last'"),
...
@pytest.mark.skipif(
    IS_JYTHON, reason="Putting unicode on frame vars does not work on Jython."
)
def test_stack_and_variables_dict(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_local_variables.py") as writer:
        ...
        variables_references = json_facade.pop_variables_reference(
            variables_response.body.variables
        )
        ...
        check = [
            x
            for x in variables_response.body.variables
            if x["name"] not in DAPGrouper.SCOPES_SORTED
        ]
        assert check == [
            {
                "name": "'a'",
                "value": "30",
                "type": "int",
                "evaluateName": "variable_for_test_3['a']",
            ...
            },
            {
                ...
                "value": "20",
                "type": "int",
                "evaluateName": "variable_for_test_3['b']",
                "variablesReference": 0,
            },
            {
...
def test_variables_with_same_name(case_setup_dap):
    with case_setup_dap.test_file(
        "_debugger_case_variables_with_same_name.py"
    ) as writer:
        json_facade = JsonFacade(writer)
        ...
        assert variables_response.body.variables == [
            {
                "name": "td",
                "value": "{foo: 'bar', gad: 'zooks', foo: 'bur'}",
                "type": "dict",
                "evaluateName": "td",
            }
        ]

        dict_variables_response = json_facade.get_variables_response(
            dict_variable_reference
        )
...
def test_hasattr_failure(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_hasattr_crash.py") as writer:
        ...
        for variable in variables_response.body.variables:
            ...
            raise AssertionError(
                'Did not find "obj" in %s' % (variables_response.body.variables,)
            )

        evaluate_response = json_facade.evaluate(
            "obj", json_hit.frame_id, context="hover"
        )
        evaluate_response_body = evaluate_response.body.to_dict()
        assert (
            evaluate_response_body["result"]
            == "An exception was raised: RuntimeError()"
        )

        json_facade.evaluate(
            "not_there", json_hit.frame_id, context="hover", success=False
        )
        json_facade.evaluate(
            "not_there", json_hit.frame_id, context="watch", success=False
        )
...
def test_warning_on_repl(case_setup_dap):
    ...
    with case_setup_dap.test_file(
        "_debugger_case_evaluate.py", additional_output_checks=additional_output_checks
    ) as writer:
        json_facade = JsonFacade(writer)
        ...
        json_facade.evaluate(
            'import warnings; warnings.warn("WarningCalledOnRepl")',
            json_hit.frame_id,
            context="repl",
        )
...
def test_evaluate_numpy(case_setup_dap, pyfile):
    ...
    with case_setup_dap.test_file(numpy_small_array_file) as writer:
        ...
        for variable in variables_response.body.variables:
            ...
            raise AssertionError(
                'Did not find "test_array" in %s' % (variables_response.body.variables,)
            )

        evaluate_response = json_facade.evaluate(
            "test_array", json_hit.frame_id, context="repl"
        )

        variables_response = json_facade.get_variables_response(
            evaluate_response.body.variablesReference
        )
...
def test_evaluate_block_repl(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_local_variables2.py") as writer:
        ...
        messages = json_facade.mark_messages(
            OutputEvent, lambda output_event: "var0" in output_event.body.output
        )
        assert len(messages) == 1
        messages = json_facade.mark_messages(
            OutputEvent, lambda output_event: "var1" in output_event.body.output
        )
        assert len(messages) == 1
        ...
        messages = json_facade.mark_messages(
            OutputEvent, lambda output_event: "foo0" in output_event.body.output
        )
        assert len(messages) == 1
        messages = json_facade.mark_messages(
            OutputEvent, lambda output_event: "foo1" in output_event.body.output
        )
        assert len(messages) == 1
...
def test_evaluate_block_clipboard(case_setup_dap, pyfile):
    ...
    with case_setup_dap.test_file(target) as writer:
        ...
        evaluate_response = json_facade.evaluate(
            "var2", frameId=json_hit.frame_id, context="clipboard", fmt={"hex": True}
        )
        ...
        assert variables_response.body.variables == [
            {
                "variablesReference": 0,
                "type": "int",
                "evaluateName": "self.__dict__[var1]",
                "name": "var1",
                "value": "10",
            }
        ]
...
def test_return_value_regular(case_setup_dap, scenario, asyncio):
    with case_setup_dap.test_file(
        "_debugger_case_return_value.py"
        if not asyncio
        else "_debugger_case_return_value_asyncio.py"
    ) as writer:
        json_facade = JsonFacade(writer)
        ...
        if scenario == "step_next":
            json_facade.write_step_next(json_hit.thread_id)
            json_hit = json_facade.wait_for_thread_stopped(
                "step", name="main", line=break_line + 1
            )
        ...
        variables_response = json_facade.get_variables_response(json_hit.frame_id)
        return_variables = json_facade.filter_return_variables(
            variables_response.body.variables
        )
...
def test_stack_and_variables_set_and_list(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_local_variables2.py") as writer:
        ...
        if IS_PYPY:
            ...
            assert cleaned_vars.groups_found == set(
                [DAPGrouper.SCOPE_SPECIAL_VARS, DAPGrouper.SCOPE_FUNCTION_VARS]
            )
...
def test_evaluate_unicode(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_local_variables.py") as writer:
        ...
        json_facade.write_set_breakpoints(
            writer.get_line_index_with_content("Break 2 here")
        )
        json_facade.write_make_initial_run()
...
def test_evaluate_exec_unicode(case_setup_dap):
    ...
    with case_setup_dap.test_file(
        "_debugger_case_local_variables2.py", get_environ=get_environ
    ) as writer:
        json_facade = JsonFacade(writer)
        ...
        messages = json_facade.mark_messages(
            OutputEvent,
            lambda output_event: ("中" in output_event.body.output)
            and ("pydevd warning" not in output_event.body.output),
        )
        assert len(messages) == 1
...
def test_evaluate_repl_redirect(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_local_variables2.py") as writer:
        ...
        messages = json_facade.mark_messages(
            OutputEvent, lambda output_event: "var" in output_event.body.output
        )
        assert len(messages) == 1
...
def test_evaluate_no_double_exec(case_setup_dap, pyfile):
    ...
    with case_setup_dap.test_file(exec_code) as writer:
        ...
        messages = json_facade.mark_messages(
            OutputEvent, lambda output_event: "Something" in output_event.body.output
        )
        assert len(messages) == 1
...
def test_evaluate_variable_references(case_setup_dap):
    ...
    with case_setup_dap.test_file("_debugger_case_local_variables2.py") as writer:
        ...
        evaluate_response = json_facade.wait_for_response(
            json_facade.write_request(
                EvaluateRequest(
                    EvaluateArguments("variable_for_test_2", json_hit.frame_id)
                )
            )
        )
        ...
        variables_reference = json_facade.pop_variables_reference(
...
def test_set_expression(case_setup_dap):
    ...
    with case_setup_dap.test_file("_debugger_case_local_variables2.py") as writer:
        ...
        assert {
            ...
            "evaluateName": "bb",
            "variablesReference": 0,
        } in variables_response.to_dict()["body"]["variables"]
...
def test_set_expression_failures(case_setup_dap):
    ...
    with case_setup_dap.test_file("_debugger_case_local_variables2.py") as writer:
        ...
        assert (
            set_expression_response.message
            == "Unable to find thread to set expression."
        )
    ...
    with case_setup_dap.test_file("_debugger_case_completions.py") as writer:
        ...
        for i in range(2):
            ...
            if i == 1:
                ...
                response = json_facade.get_variables_response(
                    first_hit.frame_id, success=False
                )
                assert (
                    response.message
...
def test_set_variable_failure(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_local_variables2.py") as writer:
        ...
        set_variable_response = json_facade.write_set_variable(
            0, "invalid_reference", "invalid_reference", success=False
        )
        assert not set_variable_response.success
        assert (
            set_variable_response.message
            == "Unable to find thread to evaluate variable reference."
        )
...
def _check_tuple(json_facade, json_hit):
    ...
    response = json_facade.write_set_variable(
        variable.variablesReference, var0.name, "1", success=False
    )
...
def _check_set(json_facade, json_hit):
    ...
    set_variables_response = json_facade.write_set_variable(
        set_var.variablesReference, var_in_set.name, "1"
    )
    ...
    response = json_facade.write_set_variable(
        set_var.variablesReference, var_in_set.name, "[22]", success=False
    )
    assert response.message.startswith("Unable to change: ")

    # Check that it's still the same (the existing entry was not removed).
    assert (
        json_facade.get_local_var(json_hit.frame_id, "variable_for_test_2").value
        == "set([1, self.var1: 12])"
    )

    set_variables_response = json_facade.write_set_variable(
        set_var.variablesReference, var_in_set.name, "(22,)"
    )
    ...
    var_in_tuple_in_set = json_facade.get_var(
        set_variables_response.body.variablesReference, "0"
    )
...
def test_stack_and_variables(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_local_variables.py") as writer:
        ...
        stack_trace_request = json_facade.write_request(
            pydevd_schema.StackTraceRequest(
                pydevd_schema.StackTraceArguments(
                    threadId=json_hit.thread_id, format={"module": True, "line": True}
                )
            )
        )
        ...
        assert stack_frame["source"]["path"].endswith(
            "_debugger_case_local_variables.py"
        )
        ...
        variables_response = json_facade.get_variables_response(
            frame_variables_reference, fmt={"hex": True}
        )
        # : :type variables_response: VariablesResponse
        assert variables_response.body.variables == [
            {
        ...
        if not IS_JYTHON:
            ...
            assert isinstance(
                set_variable_response_as_dict.pop("variablesReference"), int
            )
...
def test_hex_variables(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_local_variables_hex.py") as writer:
        ...
        stack_trace_request = json_facade.write_request(
            pydevd_schema.StackTraceRequest(
                pydevd_schema.StackTraceArguments(threadId=json_hit.thread_id)
            )
        )
        stack_trace_response = json_facade.wait_for_response(stack_trace_request)
        stack_trace_response_body = stack_trace_response.body
        ...
        assert stack_frame["name"] == "Call"
        assert stack_frame["source"]["path"].endswith(
            "_debugger_case_local_variables_hex.py"
        )
        ...
        variables_request = json_facade.write_request(
            pydevd_schema.VariablesRequest(
                pydevd_schema.VariablesArguments(frame_variables_reference, format=fmt)
            )
        )
        ...
        (
            ...
            list(
                ...
                if v["name"].startswith("variables_for_test")
            ),
            key=lambda v: v["name"],
        )
...
def test_pause_and_continue(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_pause_continue.py") as writer:
        ...
        set_variable_response_as_dict = set_variable_response.to_dict()["body"]
        assert set_variable_response_as_dict == {
            "value": "False",
            "type": "bool",
            "variablesReference": 0,
        }
...
def test_step_next_step_in_multi_threads(
    case_setup_dap, stepping_resumes_all_threads, step_mode
):
    with case_setup_dap.test_file("_debugger_case_multi_threads_stepping.py") as writer:
        json_facade = JsonFacade(writer)
        ...
        for _i in range(20):
            ...
            if local_var.value == "True":
                if stepping_resumes_all_threads:
                    ...
                    raise AssertionError(
                        "Did not expect _event2_set to be set when not resuming other threads on step."
                    )
...
def test_stepping(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_stepping.py") as writer:
        ...
        json_facade.write_step_next(json_hit.thread_id)
        json_hit = json_facade.wait_for_thread_stopped(
            "step", line=before_step_over_line + 1
        )
...
def test_evaluate(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_evaluate.py") as writer:
        ...
        eval_response = json_facade.evaluate(
            "var_1", frameId=stack_frame_id, context="repl"
        )
        ...
        exec_response = json_facade.evaluate(
            "var_1 = 6", frameId=stack_frame_id, context="repl"
        )
        ...
        exec_response = json_facade.evaluate(
            "import email;email.foo_value", success=False
        )
...
def test_evaluate_failures(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_completions.py") as writer:
        ...
        exec_request = json_facade.write_request(
            pydevd_schema.EvaluateRequest(
                pydevd_schema.EvaluateArguments("a = 10", frameId=9999, context="repl")
            )
        )
        ...
        for i in range(2):
            ...
            if i == 0:
                ...
                response = json_facade.evaluate(
                    "invalid_var",
                    frameId=first_hit.frame_id,
                    context="watch",
                    success=False,
                )
                assert (
                    response.body.result
                    == "NameError: name 'invalid_var' is not defined"
                )
            if i == 1:
                # Now, check with a previously existing frameId.
                exec_request = json_facade.write_request(
                    pydevd_schema.EvaluateRequest(
                        pydevd_schema.EvaluateArguments(
                            "a = 10", frameId=first_hit.frame_id, context="repl"
                        )
                    )
                )
                exec_response = json_facade.wait_for_response(exec_request)
...
def test_evaluate_exception_trace(case_setup_dap, pyfile):
    ...
    with case_setup_dap.test_file(exception_trace_file) as writer:
        ...
        exec_response = json_facade.evaluate(
            "method3()", json_hit.frame_id, "repl", success=False
        )
        assert (
            "pydevd" not in exec_response.message
        )  # i.e.: don't show pydevd in the trace
        assert "method3" in exec_response.message
        assert "method2" in exec_response.message

        exec_response = json_facade.evaluate(
            "method2()", json_hit.frame_id, "repl", success=False
        )
...
@pytest.mark.parametrize(
    "max_frames", ["default", "all", 10]
)  # -1 = default, 0 = all, 10 = 10 frames
    ...
    with case_setup_dap.test_file("_debugger_case_deep_stacks.py") as writer:
        ...
        while startFrame < total_frames:
            stack_trace_request = json_facade.write_request(
                pydevd_schema.StackTraceRequest(
                    pydevd_schema.StackTraceArguments(
                        threadId=json_hit.thread_id, startFrame=startFrame, levels=20
                    )
                )
            )
            stack_trace_response = json_facade.wait_for_response(stack_trace_request)
            ...
            pydevd_schema.SetBreakpointsRequest(
                pydevd_schema.SetBreakpointsArguments(
                    source=pydevd_schema.Source(
                        path=writer.TEST_FILE, sourceReference=0
                    ),
                    breakpoints=[
                        pydevd_schema.SourceBreakpoint(bp_requested).to_dict()
                    ],
                )
            )
        )
...
def test_goto(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_set_next_statement.py") as writer:
        ...
        goto_request = json_facade.write_request(
            pydevd_schema.GotoRequest(
                pydevd_schema.GotoArguments(threadId=json_hit.thread_id, targetId=12345)
            )
        )
        ...
        goto_request = json_facade.write_request(
            pydevd_schema.GotoRequest(
                pydevd_schema.GotoArguments(
                    threadId=json_hit.thread_id, targetId=target_id
                )
            )
        )
        goto_response = json_facade.wait_for_response(goto_request)
...
def _collect_stack_frames_ending_with(json_hit, end_with_pattern):
    stack_trace_response = json_hit.stack_trace_response
    dont_trace_frames = list(
        frame
        for frame in stack_trace_response.body.stackFrames
...
@pytest.mark.parametrize(
    "dbg_property", ["dont_trace", "trace", "change_pattern", "dont_trace_after_start"]
)
def test_set_debugger_property(case_setup_dap, dbg_property):
    kwargs = {}

    with case_setup_dap.test_file(
        "_debugger_case_dont_trace_test.py", **kwargs
    ) as writer:
        json_facade = JsonFacade(writer)
        ...
        if dbg_property in ("dont_trace", "change_pattern", "dont_trace_after_start"):
            json_facade.write_set_debugger_property(
                [], ["dont_trace.py"] if not IS_WINDOWS else ["Dont_Trace.py"]
            )
...
def test_source_mapping_errors(case_setup_dap):
    ...
    with case_setup_dap.test_file("_debugger_case_source_mapping.py") as writer:
        ...
        cell1_map = PydevdSourceMap(
            map_to_cell_1_line2, map_to_cell_1_line2 + 1, Source(path="<cEll1>"), 2
        )
        cell2_map = PydevdSourceMap(
            map_to_cell_2_line2, map_to_cell_2_line2 + 1, Source(path="<cEll2>"), 2
        )
...
@pytest.mark.parametrize(
    "target",
    [
        "_debugger_case_source_mapping.py",
        "_debugger_case_source_mapping_and_reference.py",
    ],
)
...
def test_source_mapping_base(case_setup_dap, target, jmc):
    ...
    with case_setup_dap.test_file(target) as writer:
        ...
        json_hit = json_facade.wait_for_thread_stopped(
            line=map_to_cell_1_line2, file=os.path.basename(test_file)
        )
        for stack_frame in json_hit.stack_trace_response.body.stackFrames:
            assert stack_frame["source"]["sourceReference"] == 0
        ...
        json_hit = json_facade.wait_for_thread_stopped(
            line=map_to_cell_2_line2, file=os.path.basename(test_file)
        )
...
def test_source_mapping_just_my_code(case_setup_dap):
    ...
    with case_setup_dap.test_file("_debugger_case_source_mapping_jmc.py") as writer:
        ...
        cell1_map = PydevdSourceMap(
            map_to_cell_1_line1, map_to_cell_1_line7, Source(path="<cEll1>"), 1
        )
        ...
        json_hit = json_facade.wait_for_thread_stopped(
            line=map_to_cell_1_line6, file=os.path.basename(test_file)
        )
        for stack_frame in json_hit.stack_trace_response.body.stackFrames:
            assert stack_frame["source"]["sourceReference"] == 0
...
def test_source_mapping_goto_target(case_setup_dap):
    ...
    with case_setup_dap.test_file(
        "_debugger_case_source_map_goto_target.py",
        additional_output_checks=additional_output_checks,
    ) as writer:
        test_file = writer.TEST_FILE
        if isinstance(test_file, bytes):
        ...
        cell1_map = PydevdSourceMap(
            map_to_cell_1_line1, map_to_cell_1_line5, Source(path="<Cell1>"), 1
        )
        pydevd_source_maps = [cell1_map]
        json_facade.write_set_pydevd_source_map(
            Source(path=test_file),
...
@pytest.mark.skipif(
    not TEST_CHERRYPY or IS_WINDOWS, reason="No CherryPy available / not ok in Windows."
)
...
"""
    ...
    with case_setup_multiprocessing_dap.test_file(
        file_to_check, get_environ=get_environ
    ) as writer:
        ...
        def _ignore_stderr_line(line):
            ...
            return (
                "ENGINE " in line
                or "CherryPy Checker" in line
                or "has an empty config" in line
            )
            ...
            def run(self):
                try:
                    ...
                    t = writer.create_request_thread(
                        "http://127.0.0.1:%s/exit" % (port,)
                    )
                    t.start()
                except Exception as e:
                    secondary_thread_errors.append(
                        "Error from secondary thread: %s" % (e,)
                    )
                    raise
        ...
        def create_msg():
            return (
                "Expected secondary thread to finish before timeout.\nSecondary thread log:\n%s\nSecondary thread errors:\n%s\n"
        ...
        if secondary_thread_errors:
            raise AssertionError(
                "Found errors in secondary thread: %s" % (secondary_thread_errors,)
            )
...
def test_wait_for_attach_debugpy_mode(case_setup_remote_attach_to_dap):
    ...
    with case_setup_remote_attach_to_dap.test_file(
        "_debugger_case_wait_for_attach_debugpy_mode.py", host_port[1]
    ) as writer:
        time.sleep(
            1
        )  # Give some time for it to pass the first breakpoint and wait in 'wait_for_attach'.
...
def test_wait_for_attach(case_setup_remote_attach_to_dap):
    ...
    with case_setup_remote_attach_to_dap.test_file(
        "_debugger_case_wait_for_attach.py", host_port[1]
    ) as writer:
        writer.TEST_FILE = debugger_unittest._get_debugger_test_file(
            "_debugger_case_wait_for_attach_impl.py"
        )
        time.sleep(
            1
        )  # Give some time for it to pass the first breakpoint and wait in 'wait_for_attach'.
        ...
        json_facade.write_pause()
        json_hit = json_facade.wait_for_thread_stopped(
            reason="pause", line=[pause1_line, pause2_line]
        )
...
def test_wait_for_attach_gevent(case_setup_remote_attach_to_dap):
    ...
    with case_setup_remote_attach_to_dap.test_file(
        "_debugger_case_gevent.py",
        host_port[1],
        additional_args=["remote", "as-server"],
        get_environ=get_environ,
    ) as writer:
        writer.TEST_FILE = debugger_unittest._get_debugger_test_file(
            "_debugger_case_gevent.py"
...
def test_gevent_show_paused_greenlets(case_setup_dap, show):
    ...
    with case_setup_dap.test_file(
        "_debugger_case_gevent_simple.py", get_environ=get_environ
    ) as writer:
        json_facade = JsonFacade(writer)
        ...
        if show:
            ...
            thread_name_to_id = dict(
                (t["name"], t["id"]) for t in response.body.threads
            )
            ...
            for tname, tid in thread_name_to_id.items():
                stack = json_facade.get_stack_as_json_hit(
                    tid, no_stack_frame=tname == "Hub: run - hub.py"
                )
                assert stack
...
def test_gevent_subprocess_not_python(case_setup_dap):
    ...
    with case_setup_dap.test_file(
        "_debugger_case_gevent_subprocess.py", get_environ=get_environ
    ) as writer:
        json_facade = JsonFacade(writer)
...
def test_path_translation_and_source_reference(case_setup_dap):
    ...
    def get_file_in_client(writer):
        ...
        return os.path.join(
            ...
            translated_dir_not_ascii,
            "_debugger_case_path_translation.py",
        )
    ...
    with case_setup_dap.test_file(
        "_debugger_case_path_translation.py", get_environ=get_environ
    ) as writer:
        ...
        assert (
            source_reference == 0
        )  # When it's translated the source reference must be == 0
        ...
        if not stack_frame_not_path_translated["name"].startswith(
            ...
            raise AssertionError(
                "Error. Found: >>%s<<." % (stack_frame_not_path_translated["name"],)
            )
...
def test_linecache_json_existing_file(case_setup_dap, tmpdir):
    with case_setup_dap.test_file(
        "_debugger_case_linecache_existing_file.py"
    ) as writer:
        json_facade = JsonFacade(writer)

        json_facade.write_launch(justMyCode=False)

        debugger_case_stepping_filename = debugger_unittest._get_debugger_test_file(
            "_debugger_case_stepping.py"
        )
        bp_line = writer.get_line_index_with_content(
            "Break here 1", filename=debugger_case_stepping_filename
        )
        json_facade.write_set_breakpoints(
            bp_line, filename=debugger_case_stepping_filename
        )
        json_facade.write_make_initial_run()
...
def test_show_bytecode_json(case_setup_dap, tmpdir):
    with case_setup_dap.test_file("_debugger_case_show_bytecode.py") as writer:
        ...
        for source_reference in source_references:
            response = json_facade.write_get_source(source_reference)
            assert (
                "MyClass" in response.body.content or "foo()" in response.body.content
            )
...
def test_case_django_no_attribute_exception_breakpoint(case_setup_django_dap, jmc):
    ...
    with case_setup_django_dap.test_file(EXPECTED_RETURNCODE="any") as writer:
        ...
        time.sleep(
            5
        )  # Give django some time to get to startup before requesting the page
        t.start()

        json_hit = json_facade.wait_for_thread_stopped(
            "exception", line=7, file="template_error.html"
        )
        ...
        entries = [
            x
            for x in variables_response.to_dict()["body"]["variables"]
            if x["name"] == "entry"
        ]
        assert len(entries) == 1
        variables_response = json_facade.get_variables_response(
            entries[0]["variablesReference"]
        )
...
def test_case_django_line_validation(case_setup_django_dap):
    ...
    with case_setup_django_dap.test_file(EXPECTED_RETURNCODE="any") as writer:
        ...
        template_file = debugger_unittest._get_debugger_test_file(
            os.path.join(
                writer.DJANGO_FOLDER, "my_app", "templates", "my_app", "index.html"
            )
        )
        file_doesnt_exist = os.path.join(
            os.path.dirname(template_file), "this_does_not_exist.html"
        )
        ...
        response = json_facade.write_set_breakpoints(
            [1], file_doesnt_exist, verified=False
        )
...
def test_case_flask_line_validation(case_setup_flask_dap):
    with case_setup_flask_dap.test_file(EXPECTED_RETURNCODE="any") as writer:
        json_facade = JsonFacade(writer)
        writer.write_set_project_roots(
            [debugger_unittest._get_debugger_test_file("flask1")]
        )
        json_facade.write_launch(debugOptions=["Jinja"])
        json_facade.write_make_initial_run()

        template_file = debugger_unittest._get_debugger_test_file(
            os.path.join("flask1", "templates", "hello.html")
        )
        ...
        time.sleep(
            2
        )  # Give flask some time to get to startup before requesting the page
        t.start()
...
def test_case_flask_exceptions(case_setup_flask_dap, jmc):
    with case_setup_flask_dap.test_file(EXPECTED_RETURNCODE="any") as writer:
        ...
        while True:
            ...
            raise AssertionError(
                "Unexpected thread stop: at %s, %s" % (path, found_line)
            )
...
@pytest.mark.skipif(
    IS_APPVEYOR or IS_JYTHON,
    reason="Flaky on appveyor / Jython encoding issues (needs investigation).",
)
def test_redirect_output(case_setup_dap):
    ...
    with case_setup_dap.test_file(
        "_debugger_case_redirect.py", get_environ=get_environ
    ) as writer:
        original_ignore_stderr_line = writer._ignore_stderr_line
...
def _attach_to_writer_pid(writer):
    ...
    def attach():
        attach_pydevd_file = os.path.join(
...
@pytest.mark.skipif(
    not IS_CPYTHON or IS_MAC,
    reason="Attach to pid only available in CPython (brittle on Mac).",
)
def test_attach_to_pid(case_setup_remote, reattach):
    import threading

    with case_setup_remote.test_file(
        "_debugger_case_attach_to_pid_simple.py", wait_for_port=False
    ) as writer:
        ...
        class SecondaryProcessThreadCommunication(threading.Thread):
            def run(self):
                ...
                for _ in range(expected_connections):
                    ...
                    self.server_socket = server_socket
                    writer.log.append(
                        "  *** Multiprocess waiting on server_socket.accept()"
                    )
                    new_sock, addr = server_socket.accept()
                    writer.log.append(
                        "  *** Multiprocess completed server_socket.accept()"
                    )
...
def test_subprocess_then_fork(case_setup_multiprocessing_dap):
    ...
    with case_setup_multiprocessing_dap.test_file(
        "_debugger_case_subprocess_and_fork.py"
    ) as writer:
        json_facade = JsonFacade(writer)
        json_facade.write_launch(justMyCode=False)
        ...
        class SecondaryProcessThreadCommunication(threading.Thread):
            def run(self):
                ...
                for i in range(2):
                    ...
                    self.server_socket = server_socket
                    writer.log.append(
                        "  *** Multiprocess %s waiting on server_socket.accept()" % (i,)
                    )
                    new_sock, addr = server_socket.accept()
                    writer.log.append(
                        "  *** Multiprocess %s completed server_socket.accept()" % (i,)
                    )
                    ...
                    reader_thread.start()
                    writer.log.append(
                        "  *** Multiprocess %s started ReaderThread" % (i,)
                    )
                    ...
                    writer.log.append(
                        "  *** Multiprocess %s write set breakpoints" % (i,)
                    )
                    json_facade2.write_set_breakpoints([break_line])
                    writer.log.append(
                        "  *** Multiprocess %s write make initial run" % (i,)
                    )
                    json_facade2.write_make_initial_run()
                ...
                for i, json_facade3 in enumerate(json_facades):
                    writer.log.append(
                        "  *** Multiprocess %s wait for thread stopped" % (i,)
                    )
...
def test_no_subprocess_patching(
    case_setup_multiprocessing_dap, apply_multiprocessing_patch
):
    import threading
    from tests_python.debugger_unittest import AbstractWriterThread
    ...
    with case_setup_multiprocessing_dap.test_file(
        ...
        if apply_multiprocessing_patch:
            secondary_process_thread_communication = (
                SecondaryProcessThreadCommunication()
            )
            secondary_process_thread_communication.start()
            time.sleep(0.1)
...
def test_module_crash(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_module.py") as writer:
        ...
        module_event = json_facade.wait_for_json_message(
            ModuleEvent
        )  # : :type module_event: ModuleEvent
...
def test_pydevd_systeminfo(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_print.py") as writer:
        ...
        if use_cython is not None:
            ...
            assert body["pydevd"]["usingFrameEval"] == (
                using_cython and IS_PY36_OR_GREATER and not TODO_PY311
            )
...
def test_terminate(case_setup_dap, scenario, check_subprocesses):
    ...
    with case_setup_dap.test_file(
        ...
        update_command_line_args=update_command_line_args,
        EXPECTED_RETURNCODE="any"
        if check_subprocesses == "kill_subprocesses_ignore_pid"
        else 0,
    ) as writer:
        json_facade = JsonFacade(writer)
...
def test_access_token(case_setup_dap):
    ...
    with case_setup_dap.test_file(
        "_debugger_case_pause_continue.py",
        update_command_line_args=update_command_line_args,
    ) as writer:
        json_facade = JsonFacade(writer)

        response = json_facade.write_set_debugger_property(
            multi_threads_single_notification=True, success=False
        )
        ...
        authorize_response = json_facade.write_authorize(
            access_token="bar123", success=True
        )
        ...
        break_line = writer.get_line_index_with_content(
            "Pause here and change loop to False"
        )
        json_facade.write_set_breakpoints(break_line)
        json_facade.write_make_initial_run()
...
def test_debug_options(case_setup_dap, val):
    with case_setup_dap.test_file("_debugger_case_debug_options.py") as writer:
        ...
        output = json_facade.wait_for_json_message(
            OutputEvent,
            lambda msg: msg.body.category == "stdout"
            and msg.body.output.startswith("{")
            and msg.body.output.endswith("}"),
        )
        ...
        assert json.loads(output.body.output) == dict(
            (translation[key], val) for key, val in args.items()
        )
        json_facade.wait_for_terminated()
        writer.finished_ok = True
...
def test_gui_event_loop_custom(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_gui_event_loop.py") as writer:
        json_facade = JsonFacade(writer)
        json_facade.write_launch(
            guiEventLoop="__main__.LoopHolder.gui_loop", redirectOutput=True
        )
        break_line = writer.get_line_index_with_content("break here")
        json_facade.write_set_breakpoints(break_line)
        ...
        json_facade.wait_for_json_message(
            OutputEvent,
            lambda msg: msg.body.category == "stdout"
            and "gui_loop() called" in msg.body.output,
        )
    ...
    with case_setup_dap.test_file("_debugger_case_gui_event_loop_qt5.py") as writer:
        ...
        for _i in range(3):
            json_facade.wait_for_json_message(
                OutputEvent,
                lambda msg: msg.body.category == "stdout"
                and "on_timeout() called" in msg.body.output,
            )
...
def test_just_my_code_debug_option_deprecated(
    case_setup_dap, debug_stdlib, debugger_runner_simple
):
...
def test_send_invalid_messages(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_local_variables.py") as writer:
        ...
        response = json_facade.wait_for_response(
            json_facade.write_request({"type": "request", "command": "modules"})
        )
        ...
        response = json_facade.wait_for_response(
            json_facade.write_request({"type": "request", "command": "completions"})
        )
        assert not response.success
...
def test_send_json_message(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_custom_message.py") as writer:
        ...
        json_facade.wait_for_json_message(
            OutputEvent,
            lambda msg: msg.body.category == "my_category"
            and msg.body.output == "some output",
        )

        json_facade.wait_for_json_message(
            OutputEvent,
            lambda msg: msg.body.category == "my_category2"
            and msg.body.output == "some output 2",
        )
...
def test_global_scope(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_globals.py") as writer:
        json_facade = JsonFacade(writer)
        json_facade.write_set_breakpoints(
            writer.get_line_index_with_content("breakpoint here")
        )
...
def _check_class_group_special_inline_presentation(
    json_facade, json_hit, variables_response
):
    var_names = [v["name"] for v in variables_response.body.variables]
    assert var_names[:3] == ["class variables", "in_global_scope", "__builtins__"]

    variables_response = json_facade.get_variables_response(
        variables_response.body.variables[0]["variablesReference"]
    )
    var_names = [v["name"] for v in variables_response.body.variables]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"class": "group", "special": "inline"},
            _check_class_group_special_inline_presentation,
        ),
    ],
)
def test_variable_presentation(case_setup_dap, var_presentation, check_func):
    with case_setup_dap.test_file("_debugger_case_globals.py") as writer:
        ...
        variables_response = json_facade.get_variables_response(
            name_to_scope["Globals"].variablesReference
        )
...
def test_debugger_case_deadlock_thread_eval(case_setup_dap):
    ...
    with case_setup_dap.test_file(
        "_debugger_case_deadlock_thread_eval.py", get_environ=get_environ
    ) as writer:
        json_facade = JsonFacade(writer)
        json_facade.write_launch()
        json_facade.write_set_breakpoints(
        ...
        json_facade.evaluate(
            'processor.process("process in evaluate")', json_hit.frame_id
        )
...
def test_debugger_case_breakpoint_on_unblock_thread_eval(case_setup_dap):
    ...
    with case_setup_dap.test_file(
        ...
        evaluate_request = json_facade.evaluate(
            'processor.process("process in evaluate")',
            json_hit.frame_id,
        ...
        messages = json_facade.mark_messages(
            EvaluateResponse,
            lambda evaluate_response: evaluate_response.request_seq
            == evaluate_request.seq,
        )
        assert len(messages) == 1
...
def test_debugger_case_unblock_manually(case_setup_dap):
    ...
    with case_setup_dap.test_file(
        ...
        json_facade.wait_for_json_message(
            OutputEvent,
            lambda output_event: "did not finish after" in output_event.body.output,
        )
...
def test_debugger_case_deadlock_notify_evaluate_timeout(case_setup_dap, pyfile):
    ...
    with case_setup_dap.test_file(
        case_slow_evaluate, get_environ=get_environ
    ) as writer:
        json_facade = JsonFacade(writer)
        json_facade.write_launch(justMyCode=False)
        json_facade.write_set_breakpoints(
...
def test_debugger_case_deadlock_interrupt_thread(case_setup_dap, pyfile):
    ...
    with case_setup_dap.test_file(
        case_infinite_evaluate, get_environ=get_environ, EXPECTED_RETURNCODE="any"
    ) as writer:
        json_facade = JsonFacade(writer)
        json_facade.write_launch(justMyCode=False)
        json_facade.write_set_breakpoints(
        ...
        # If threads aren't resumed, this will deadlock.
        json_facade.evaluate(
            "infinite_evaluate()", json_hit.frame_id, wait_for_response=False
        )
...
def test_debugger_case_symlink(
    case_setup_dap, tmpdir, launch_through_link, breakpoints_through_link
):
    """
    ...
    try:
        os.symlink(
            os.path.dirname(original_filename), target_link, target_is_directory=True
        )
    ...
    try:
        ...
        with case_setup_dap.test_file(
            target_filename_in_link if launch_through_link else original_filename
        ) as writer:
            json_facade = JsonFacade(writer)
            json_facade.write_launch(justMyCode=False)
            ...
            json_facade.write_set_breakpoints(
                ...
                filename=target_filename_in_link
                if breakpoints_through_link
            ...
            assert (
                path == target_filename_in_link
                if launch_through_link
                else original_filename
            )
...
@pytest.mark.skipif(
    ...
    or not IS_CPYTHON
    or not TEST_CYTHON
    or TODO_PY311,  # Requires frame-eval mode (still not available for Python 3.11).
...
def test_code_reload(case_setup_dap, pyfile):
    ...
    def mod1():
        ...
        for _ in range(50):
            ...
            raise AssertionError(
                "It seems the reload was not done in the available amount of time."
            )
    ...
    with case_setup_dap.test_file(mod1) as writer:
        ...
        json_facade.write_launch(
            justMyCode=False, autoReload={"pollingInterval": 0, "enable": True}
        )
...
def test_step_into_target_basic(case_setup_dap):
    with case_setup_dap.test_file("_debugger_case_smart_step_into.py") as writer:
        ...
        label_to_id = dict(
            (target["label"], target["id"]) for target in step_in_targets
        )
...
def test_pandas(case_setup_dap, pyfile):
    ...
    def pandas_mod():
        ...
        items = rows * cols
        df = pd.DataFrame(np.arange(items).reshape(rows, cols)).applymap(
            lambda x: "Test String"
        )
        series = df._series[0]
        styler = df.style
    ...
    with case_setup_dap.test_file(pandas_mod) as writer:
        ...
        evaluate_response = json_facade.evaluate(
            "df", json_hit.frame_id, context="repl"
        )
        evaluate_response_body = evaluate_response.body.to_dict()
        ...
        df_variables_response = json_facade.get_variables_response(
            name_to_var["df"].variablesReference
        )
        ...
        df_variables_response = json_facade.get_variables_response(
            name_to_var["series"].variablesReference
        )
        for v in df_variables_response.body.variables:
            if v["name"] == "T":
                assert v["value"] == "'<transposed dataframe -- debugger:skipped eval>'"
        ...
        df_variables_response = json_facade.get_variables_response(
            name_to_var["styler"].variablesReference
        )
        for v in df_variables_response.body.variables:
            if v["name"] == "data":
...
@pytest.mark.skipif(
    not IS_PY38_OR_GREATER, reason="Python 3.8 onwards required for test."
)
def test_same_lineno_and_filename(case_setup_dap, pyfile):
    ...
    def target():
        ...
        code_obj = code_obj.replace(
            co_name=some_code.__code__.co_name,
            co_firstlineno=some_code.__code__.co_firstlineno,
        )
...
def test_ipython_stepping_basic(case_setup_dap):
    def get_environ(self):
        ...
        ] = "_debugger_case_scoped_stepping.py, run_code, run_ast_nodes"
        return env

    with case_setup_dap.test_file(
        "_debugger_case_scoped_stepping.py", get_environ=get_environ
    ) as writer:
        json_facade = JsonFacade(writer)
        json_facade.write_launch(justMyCode=False)

        target_file = debugger_unittest._get_debugger_test_file(
            "_debugger_case_scoped_stepping_target.py"
        )
        ...
        json_facade.write_make_initial_run()
        json_hit = json_facade.wait_for_thread_stopped(
            line=break_line, file="_debugger_case_scoped_stepping_target.py"
        )

        json_facade.write_step_next(json_hit.thread_id)
        json_hit = json_facade.wait_for_thread_stopped(
            "step", line=break_line + 1, file="_debugger_case_scoped_stepping_target.py"
        )

        json_facade.write_step_next(json_hit.thread_id)
        json_hit = json_facade.wait_for_thread_stopped(
            "step", line=break_line + 2, file="_debugger_case_scoped_stepping_target.py"
        )
...
def test_ipython_stepping_step_in(case_setup_dap):
    ...
    with case_setup_dap.test_file(
        ...
        target_file = debugger_unittest._get_debugger_test_file(
            "_debugger_case_scoped_stepping_target2.py"
        )
        break_line = writer.get_line_index_with_content(
            "break here", filename=target_file
        )
        json_facade.write_set_breakpoints(break_line, filename=target_file)
        json_facade.write_make_initial_run()
        json_hit = json_facade.wait_for_thread_stopped(
            line=break_line, file="_debugger_case_scoped_stepping_target2.py"
        )

        json_facade.write_step_in(json_hit.thread_id)
        stop_at = writer.get_line_index_with_content("b = 2", filename=target_file)
        json_hit = json_facade.wait_for_thread_stopped(
            "step", line=stop_at, file="_debugger_case_scoped_stepping_target2.py"
        )

        json_facade.write_step_in(json_hit.thread_id)
        stop_at = writer.get_line_index_with_content(
            "method()  # break here", filename=target_file
        )
        json_hit = json_facade.wait_for_thread_stopped(
            "step", line=stop_at, file="_debugger_case_scoped_stepping_target2.py"
        )
...
def test_ipython_stepping_step_in_justmycode(case_setup_dap):
    ...
    with case_setup_dap.test_file(
        ...
        json_facade.write_make_initial_run()
        json_hit = json_facade.wait_for_thread_stopped(
            line=break_line, file="_debugger_case_scoped_stepping_print.py"
        )

        json_facade.write_step_in(json_hit.thread_id)
        stop_at = writer.get_line_index_with_content("pause 1", filename=target_file)
        json_hit = json_facade.wait_for_thread_stopped(
            "step", line=stop_at, file="_debugger_case_scoped_stepping_print.py"
        )
...
def test_logging_api(case_setup_multiprocessing_dap, tmpdir):
    ...
    with case_setup_multiprocessing_dap.test_file(
        "_debugger_case_logging.py", get_environ=get_environ
    ) as writer:
        json_facade = JsonFacade(writer)
        json_facade.write_launch()
...
def test_soft_terminate(case_setup_dap, pyfile, soft_kill):
    ...
    with case_setup_dap.test_file(
        ...
        json_facade = JsonFacade(writer)
        json_facade.write_launch(
            onTerminate="KeyboardInterrupt" if soft_kill else "kill", justMyCode=False
        )
        ...
        if soft_kill:
            json_facade.wait_for_json_message(
                OutputEvent,
                lambda output_event: "raised from within the callback set"
                in output_event.body.output,
            )
