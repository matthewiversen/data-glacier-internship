...
if __name__ == "__main__":
    ...
    def new_thread_function():
        ...
        with lock:
            initialized[0] = True
            while wait:
                time.sleep(0.1)  # break thread here
    ...
    with (
        lock
    ):  # It'll be here until the secondary thread finishes (i.e.: releases the lock).
        pass
    ...
    if curr_thread_ident != main_thread_ident:
        raise AssertionError(
            "Expected current thread ident (%s) to be the main thread ident (%s)"
