...
def test_suspended_frames_manager():
    ...
    with suspended_frames_manager.track_frames(py_db) as tracker:
        ...
        as_dict = dict(
            (x.get_name(), x.get_var_data()) for x in variable.get_children_variables()
        )
        var_reference = as_dict["var2"].pop("variablesReference")
        assert isinstance(
            var_reference, int_types
        )  # The variable reference is always a new int.
        assert isinstance(
            as_dict["var3"].pop("variablesReference"), int_types
        )  # The variable reference is always a new int.
        ...
        as_dict = dict(
            (x.get_name(), x.get_var_data(fmt={"hex": True}))
        ...
        as_dict = dict(
            [x.get_name(), x.get_var_data()]
            for x in children_vars
            if x.get_name() not in DAPGrouper.SCOPES_SORTED
        )
        assert as_dict == {
            "0": {
                "name": "0",
        ...
        assert isinstance(
            as_dict["33"].pop("variablesReference"), int_types
        )  # The variable reference is always a new int.
        ...
        with suspended_frames_manager.track_frames(py_db) as tracker:
            ...
            if not found_too_large:
                raise AssertionError(
                    "Expected to find variable named: %s" % (TOO_LARGE_ATTR,)
                )
            if not found_len:
