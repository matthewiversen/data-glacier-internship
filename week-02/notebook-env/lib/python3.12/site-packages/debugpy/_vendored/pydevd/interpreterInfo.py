...
try:
    ...
    exec(
        "True, False = 1,0"
    )  # An exec is used so that python 3k does not give a syntax error
...
if __name__ == "__main__":
    ...
    try:
        path_used = path_used[
            1:
        ]  # Use a copy (and don't include the directory of this script as a path.)
    except:
        pass  # just ignore it...
    ...
    for builtinMod in sys.builtin_module_names:
        contents.append(
            tounicode("<forced_lib>%s</forced_lib>") % tounicode(builtinMod)
        )
