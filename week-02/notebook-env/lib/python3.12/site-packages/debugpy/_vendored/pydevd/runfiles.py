...
def main():
    ...
    for arg in sys.argv[1:]:
        if (
            not found_other_test_framework_param
            and arg != NOSE_PARAMS
            and arg != PY_TEST_PARAMS
        ):
    ...
    try:
        if found_other_test_framework_param:
            if found_other_test_framework_param == NOSE_PARAMS:
                ...
                raise ImportError(
                    "Test framework: %s not supported."
    ...
    if not test_framework:
        return pydev_runfiles.main(
            configuration
        )  # Note: still doesn't return a proper value.
            ...
            if configuration.tests:
                ...
                for file in configuration.files_or_dirs:
                    if test_framework == NOSE_FRAMEWORK:
                        ...
                        py_test_accept_filter[
                            canonical_normalized_path(file)
                        ] = configuration.tests
                        files_or_dirs.append(file)
        ...
        if test_framework == NOSE_FRAMEWORK:
            ...
            PYDEV_NOSE_PLUGIN_SINGLETON = (
                pydev_runfiles_nose.start_pydev_nose_plugin_singleton(configuration)
            )
            ...
            if (
                "--coverage_output_dir" in pydev_params
                and "--coverage_include" in pydev_params
            ):
            ...
            raise AssertionError(
                "Cannot handle test framework: %s at this point." % (test_framework,)
            )
...
if __name__ == "__main__":
    ...
    if hasattr(sys, "_current_frames") and hasattr(threading, "enumerate"):
        ...
        dump_current_frames_thread.daemon = (
            True  # Daemon so that this thread doesn't halt it!
        )
