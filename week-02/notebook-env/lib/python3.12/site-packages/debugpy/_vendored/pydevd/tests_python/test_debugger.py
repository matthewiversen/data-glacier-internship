...
from _pydevd_bundle.pydevd_constants import IS_WINDOWS, IS_PY38_OR_GREATER, IS_MAC
...
@pytest.mark.skipif(
    not IS_CPYTHON,
    reason="Test needs gc.get_referrers/reference counting to really check anything.",
)
...
def test_case_2(case_setup):
    with case_setup.test_file("_debugger_case2.py") as writer:
        ...
        writer.write_get_frame(
            thread_id, frame_id
        )  # Note: write get frame but not waiting for it to be gotten.
...
@pytest.mark.parametrize(
    ...
    (
        ...
        [
            ["Exception"],
            ["ValueError"],
        ],  # ValueError will also suspend/print since we're dealing with a NameError
    ),
)
def test_case_breakpoint_condition_exc(
    ...
    with case_setup.test_file("_debugger_case_breakpoint_condition_exc.py") as writer:
        ...
        writer.write_suspend_on_breakpoint_exception(
            skip_suspend_on_breakpoint_exception, skip_print_breakpoint_exception
        )
        ...
        def check_error_msg(stderr):
            ...
            for msg in msgs_one_in_stderr:
                ...
                raise AssertionError(
                    "Did not find any of: %s in stderr: %s"
...
def test_case_double_remove_breakpoint(case_setup):
    with case_setup.test_file("_debugger_case_remove_breakpoint.py") as writer:
        ...
        hit = writer.wait_for_breakpoint_hit()
        writer.write_remove_breakpoint(breakpoint_id)
        writer.write_remove_breakpoint(
            breakpoint_id
        )  # Double-remove (just check that we don't have an error).
        writer.write_run_thread(hit.thread_id)
...
@pytest.mark.skipif(
    IS_IRONPYTHON,
    reason="This test fails once in a while due to timing issues on IronPython, so, skipping it.",
)
...
def test_case_suspend_thread(case_setup):
    with case_setup.test_file("_debugger_case4.py") as writer:
        ...
        while True:
            hit = writer.wait_for_breakpoint_hit(
                (REASON_THREAD_SUSPEND, REASON_STOP_ON_BREAKPOINT)
            )
            if hit.name == "sleep":
        ...
        writer.write_evaluate_expression(
            "%s\t%s\t%s" % (hit.thread_id, hit.frame_id, "LOCAL"), "exit_while_loop()"
        )
        writer.wait_for_evaluation(
            [
                [
                    '<var name="exit_while_loop()" type="str" qualifier="{0}" value="str: ok'.format(
                        builtin_qualifier
                    ),
...
@pytest.mark.skipif(
    IS_JYTHON, reason="Jython can only have one thread stopped at each time."
)
def test_case_suspend_all_thread(case_setup):
    with case_setup.test_file("_debugger_case_suspend_all.py") as writer:
        ...
        writer.write_evaluate_expression(
            "%s\t%s\t%s" % (hit0.thread_id, hit0.frame_id, "LOCAL"),
            "exit_while_loop(1)",
        )
        writer.wait_for_evaluation(
            [
                [
                    '<var name="exit_while_loop(1)" type="str" qualifier="{0}" value="str: ok'.format(
                        builtin_qualifier
                    )
                ]
            ]
        ...
        writer.write_evaluate_expression(
            "%s\t%s\t%s" % (hit1.thread_id, hit1.frame_id, "LOCAL"),
            "exit_while_loop(2)",
        )
        writer.wait_for_evaluation(
            '<var name="exit_while_loop(2)" type="str" qualifier="{0}" value="str: ok'.format(
                builtin_qualifier
            )
        )
...
@pytest.mark.skipif(
    IS_IRONPYTHON, reason="This test is flaky on Jython, so, skipping it."
)
def test_case_7(case_setup):
    ...
    with case_setup.test_file("_debugger_case_local_variables.py") as writer:
        writer.write_add_breakpoint(
            writer.get_line_index_with_content("Break here"), "Call"
        )
        writer.write_make_initial_run()
...
def test_case_12(case_setup):
    ...
    with case_setup.test_file("_debugger_case_simple_calls.py") as writer:
        ...
        hit = writer.wait_for_breakpoint_hit(
            "111", line=6 if IS_JYTHON else 2
        )  # not a return (it stopped in the other breakpoint)
...
@pytest.mark.skipif(
    IS_IRONPYTHON, reason="Failing on IronPython (needs to be investigated)."
)
def test_case_13(case_setup):
    with case_setup.test_file("_debugger_case13.py") as writer:
        ...
        writer.write(
            "%s\t%s\t%s"
            % (CMD_SET_PROPERTY_TRACE, writer.next_seq(), "true;false;false;true")
        )
        writer.write_make_initial_run()
        hit = writer.wait_for_breakpoint_hit("111")
        ...
        writer.write(
            "%s\t%s\t%s"
            % (CMD_SET_PROPERTY_TRACE, writer.next_seq(), "true;true;true;true")
        )
        writer.write_step_in(hit.thread_id)
...
def test_case_14(case_setup):
    ...
    with case_setup.test_file("_debugger_case14.py") as writer:
        ...
        writer.write_debug_console_expression(
            "%s\t%s\tEVALUATE\tcarObj.color" % (hit.thread_id, hit.frame_id)
        )
        ...
        writer.write_debug_console_expression(
            "%s\t%s\tEVALUATE\tcarObj.color='Red'" % (hit.thread_id, hit.frame_id)
        )
        writer.write_debug_console_expression(
            "%s\t%s\tEVALUATE\tcarObj.color" % (hit.thread_id, hit.frame_id)
        )
        ...
        # Iterate some loop
        writer.write_debug_console_expression(
            "%s\t%s\tEVALUATE\tfor i in range(3):" % (hit.thread_id, hit.frame_id)
        )
        writer.wait_for_var(["<xml><more>True</more></xml>"])
        writer.write_debug_console_expression(
            "%s\t%s\tEVALUATE\t    print(i)" % (hit.thread_id, hit.frame_id)
        )
        writer.wait_for_var(["<xml><more>True</more></xml>"])
        writer.write_debug_console_expression(
            "%s\t%s\tEVALUATE\t" % (hit.thread_id, hit.frame_id)
        )
        writer.wait_for_var(
            [
...
def test_case_16_resolve_numpy_array(case_setup):
    ...
    with case_setup.test_file("_debugger_case16.py") as writer:
        ...
        writer.write_get_variable(
            hit.thread_id, hit.frame_id, "smallarray\t__internals__"
        )
        ...
        writer.wait_for_multiple_vars(
            (
                [
                    ...
                    '<var name="size" type="int" qualifier="{0}" value="int%3A 100000"'.format(
                        builtin_qualifier
                    ),
                ],
                [
        ...
        writer.write_get_variable(
            hit.thread_id, hit.frame_id, "bigarray\t__internals__"
        )
        writer.wait_for_var('<var name="%27size%27')
        ...
        writer.write_get_variable(
            hit.thread_id, hit.frame_id, "hugearray\t__internals__"
        )
        writer.wait_for_var('<var name="%27size%27')
...
def test_case_17(case_setup):
    ...
    with case_setup.test_file("_debugger_case17.py") as writer:
        ...
        writer.write_add_breakpoint(
            writer.get_line_index_with_content("break1"), "main"
        )
        writer.write_add_breakpoint(
            writer.get_line_index_with_content("break2"), "main"
        )
        writer.write_add_breakpoint(
            writer.get_line_index_with_content("break3"), "main"
        )
        writer.write_add_breakpoint(
            writer.get_line_index_with_content("break4"), "main"
        )
        writer.write_make_initial_run()
...
def test_case_17a(case_setup):
    ...
    with case_setup.test_file("_debugger_case17a.py") as writer:
        ...
        hit = writer.wait_for_breakpoint_hit(
            REASON_STOP_ON_BREAKPOINT, line=break1_line
        )
...
def test_case_19(case_setup):
    ...
    with case_setup.test_file("_debugger_case19.py") as writer:
        ...
        writer.write_evaluate_expression(
            "%s\t%s\t%s" % (hit.thread_id, hit.frame_id, "LOCAL"), "a.__var"
        )
        writer.wait_for_evaluation(
            [
                [
                    '<var name="a.__var" type="int" qualifier="{0}" value="int'.format(
                        builtin_qualifier
                    ),
...
@pytest.mark.skipif(
    IS_JYTHON, reason="Monkey-patching related to starting threads not done on Jython."
)
...
def test_case_django_a(case_setup_django):
    ...
    with case_setup_django.test_file(
        EXPECTED_RETURNCODE="any", get_environ=get_environ
    ) as writer:
        writer.write_make_initial_run()
        ...
        if contents != "<ul><li>v1:v1</li><li>v2:v2</li></ul>":
            raise AssertionError(
                "%s != <ul><li>v1:v1</li><li>v2:v2</li></ul>" % (contents,)
            )
...
def test_case_django_b(case_setup_django):
    with case_setup_django.test_file(EXPECTED_RETURNCODE="any") as writer:
        ...
        writer.write_get_frame(hit.thread_id, hit.frame_id)
        writer.wait_for_var(
            '<var name="form" type="NameForm" qualifier="my_app.forms" value="NameForm%253A'
        )
        writer.write_run_thread(hit.thread_id)
        writer.finished_ok = True
...
def test_case_django_no_attribute_exception_breakpoint(case_setup_django, jmc):
    ...
    with case_setup_django.test_file(EXPECTED_RETURNCODE="any", **kwargs) as writer:
        ...
        hit = writer.wait_for_breakpoint_hit(
            REASON_CAUGHT_EXCEPTION, line=7, file="template_error.html"
        )

        writer.write_get_frame(hit.thread_id, hit.frame_id)
        writer.wait_for_var(
            '<var name="entry" type="Entry" qualifier="my_app.views" value="Entry: v1:v1" isContainer="True"'
        )
...
@pytest.mark.skipif(not TEST_DJANGO, reason="No django available")
def test_case_django_no_attribute_exception_breakpoint_and_regular_exceptions(
    case_setup_django,
):
    with case_setup_django.test_file(EXPECTED_RETURNCODE="any") as writer:
        writer.write_add_exception_breakpoint_django()
...
def test_case_django_invalid_template_exception_breakpoint(case_setup_django, jmc):
    ...
    with case_setup_django.test_file(EXPECTED_RETURNCODE="any", **kwargs) as writer:
        ...
        hit = writer.wait_for_breakpoint_hit(
            REASON_CAUGHT_EXCEPTION, line=4, file="template_error2.html"
        )

        writer.write_get_frame(hit.thread_id, hit.frame_id)
        writer.wait_for_var(
            '<var name="token" type="Token" qualifier="django.template.base" value="Token:'
        )
...
def test_case_qthread1(case_setup):
    with case_setup.test_file("_debugger_case_qthread1.py") as writer:
        breakpoint_id = writer.write_add_breakpoint(
            writer.get_line_index_with_content("break here"), "run"
        )
        writer.write_make_initial_run()
...
def test_case_qthread2(case_setup):
    with case_setup.test_file("_debugger_case_qthread2.py") as writer:
        breakpoint_id = writer.write_add_breakpoint(
            writer.get_line_index_with_content("break here"), "long_running"
        )
        writer.write_make_initial_run()
    ...
    with case_setup.test_file("_debugger_case_qthread4.py") as writer:
        ...
        def additional_output_checks(stdout, stderr):
            ...
            if "On start called" not in stdout:
                raise AssertionError(
                    'Expected "On start called" to be in stdout:\n%s' % (stdout,)
                )
            if "Done sleeping" not in stdout:
                raise AssertionError(
                    'Expected "Done sleeping" to be in stdout:\n%s' % (stdout,)
                )
            if "native Qt signal is not callable" in stderr:
                raise AssertionError(
                    'Did not expect "native Qt signal is not callable" to be in stderr:\n%s'
                    % (stderr,)
                )

        breakpoint_id = writer.write_add_breakpoint(
            28, "on_start"
        )  # breakpoint on print('On start called2').
        writer.write_make_initial_run()
...
def test_check_tracer_with_exceptions(case_setup):
    ...
    with case_setup.test_file(
        "_debugger_case_check_tracer.py", get_environ=get_environ
    ) as writer:
...
def test_case_handled_and_unhandled_exception_generator(
    case_setup, target_file, unhandled
):
    ...
    with case_setup.test_file(
        ...
        hit = writer.wait_for_breakpoint_hit(
            REASON_UNCAUGHT_EXCEPTION if unhandled else REASON_CAUGHT_EXCEPTION
        )
        ...
        msg = writer.wait_for_message(
            accept_message=lambda msg: 'exc_type="' in msg and 'exc_desc="' in msg,
            unquote_msg=False,
        )

        frame_names = [
            unquote(f["name"]).replace("&lt;", "<").replace("&gt;", ">")
            for f in msg.thread.frame
        ]
        ...
        if not unhandled:
            ...
            for expected_line in expected_lines:
                ...
                writer.write_get_current_exception(hit.thread_id)
                msg = writer.wait_for_message(
                    accept_message=lambda msg: 'exc_type="' in msg
                    and 'exc_desc="' in msg,
                    unquote_msg=False,
                )
    ...
    with case_setup.test_file(
        ...
        def check(hit, exc_type, exc_desc):
            ...
            if len(msg.thread.frame) == 0:
                assert unquote(unquote(msg.thread.frame["file"])).endswith(
                    "_debugger_case_unhandled_exceptions.py"
                )
            else:
                assert unquote(unquote(msg.thread.frame[0]["file"])).endswith(
                    "_debugger_case_unhandled_exceptions.py"
                )
            writer.write_run_thread(hit.thread_id)
...
def test_unhandled_exceptions_in_top_level3(case_setup_unhandled_exceptions):
    with case_setup_unhandled_exceptions.test_file(
        "_debugger_case_unhandled_exceptions_on_top_level.py", EXPECTED_RETURNCODE=1
    ) as writer:
        ...
        writer.write_add_exception_breakpoint_with_policy(
            "Exception", "1", "1", ignore_libraries="1"
        )
        writer.write_make_initial_run()
...
def test_case_set_next_statement(case_setup):
    with case_setup.test_file("_debugger_case_set_next_statement.py") as writer:
        ...
        hit = writer.wait_for_breakpoint_hit(
            REASON_STOP_ON_BREAKPOINT, line=6
        )  # Stop in line a=3 (before setting it)

        writer.write_evaluate_expression(
            "%s\t%s\t%s" % (hit.thread_id, hit.frame_id, "LOCAL"), "a"
        )
        writer.wait_for_evaluation(
            '<var name="a" type="int" qualifier="{0}" value="int: 2"'.format(
                builtin_qualifier
            )
        )
        ...
        writer.wait_for_evaluation(
            '<var name="a" type="int" qualifier="{0}" value="int: 1"'.format(
                builtin_qualifier
            )
        )
...
def test_unhandled_exceptions_get_stack(case_setup_unhandled_exceptions):
    with case_setup_unhandled_exceptions.test_file(
        ...
        assert msg.thread.frame["name"] == "<module>"
        assert msg.thread.frame["line"] == str(
...
def test_case_get_next_statement_targets(case_setup):
    with case_setup.test_file("_debugger_case_get_next_statement_targets.py") as writer:
        ...
        assert targets == expected, "Expected targets to be %s, was: %s" % (
            expected,
            targets,
        )
...
@pytest.mark.skipif(
    IS_IRONPYTHON or IS_JYTHON,
    reason="Failing on IronPython and Jython (needs to be investigated).",
)
def test_case_type_ext(case_setup):
    ...
    def get_environ(self):
        ...
        env["PYTHONPATH"] = (
            ext_base + os.pathsep + python_path if python_path else ext_base
        )
        return env

    with case_setup.test_file(
        "_debugger_case_type_ext.py", get_environ=get_environ
    ) as writer:
        ...
        assert writer.wait_for_var(
            r'<var name="area" type="int" qualifier="{0}" value="int%253A 50" />'.format(
                builtin_qualifier
            )
        )
        writer.write_run_thread(hit.thread_id)
...
def test_case_variable_access(case_setup, pyfile, data_regression):
    ...
    with case_setup.test_file(case_custom) as writer:
        ...
        assert (
            unquote_plus(obj_var["value"])
            == "<class 'list'>: [(0, 1, 2, 3, 4, 5, 6, 7, 8), [(0, 1, 2, 3, 4)]]"
        )
        ...
        def collect_vars(locator, level=0):
            writer.write(
                "%s\t%s\t%s\t%s"
                % (CMD_GET_VARIABLE, writer.next_seq(), hit.thread_id, locator)
            )
        ...
        for level, val, _locator in collect_vars("%s\tFRAME\tobj" % hit.frame_id):
            found.append(
                (("    " * level) + val["name"] + ": " + unquote_plus(val["value"]))
            )
        ...
        msg = writer.wait_for_untangled_message(
            double_unquote=True,
            accept_message=lambda cmd_id, _untangled: cmd_id
            == CMD_RUN_CUSTOM_OPERATION,
        )
        ...
        for v in msg_vars:
            ...
            raise AssertionError(
                "Unable to find ref with list[0]. Found: %s" % (msg_vars,)
            )
...
def test_case_event_ext(case_setup):
    ...
    with case_setup.test_file(
        "_debugger_case_event_ext.py", get_environ=get_environ
    ) as writer:
        original_additional_output_checks = writer.additional_output_checks
...
@pytest.mark.skipif(
    IS_JYTHON,
    reason="Jython does not seem to be creating thread started inside tracing (investigate).",
)
def test_case_writer_creation_deadlock(case_setup):
    ...
    with case_setup.test_file("_debugger_case_thread_creation_deadlock.py") as writer:
        ...
        writer.write_evaluate_expression(
            "%s\t%s\t%s" % (hit.thread_id, hit.frame_id, "LOCAL"), "create_thread()"
        )
        writer.wait_for_evaluation(
            '<var name="create_thread()" type="str" qualifier="{0}" value="str: create_thread:ok'.format(
                builtin_qualifier
            )
        )
        writer.write_run_thread(hit.thread_id)
...
def test_case_skip_breakpoints_in_exceptions(case_setup):
    ...
    with case_setup.test_file(
        "_debugger_case_skip_breakpoint_in_exceptions.py"
    ) as writer:
...
@pytest.mark.skipif(
    IS_JYTHON, reason="Not working on Jython (needs to be investigated)."
)
def test_case_handled_exceptions1(case_setup):
    ...
    with case_setup.test_file(
        "_debugger_case_exceptions.py", get_environ=get_environ
    ) as writer:
        writer.write_add_exception_breakpoint_with_policy(
            "IndexError",
        ...
        def check(hit):
            ...
            writer.wait_for_message(
                accept_message=lambda msg: "__exception__" in msg
                and "IndexError" in msg,
                unquote_msg=False,
            )
            writer.write_get_current_exception(hit.thread_id)
            msg = writer.wait_for_message(
                accept_message=lambda msg: "IndexError" in msg
                and 'exc_type="' in msg
                and 'exc_desc="' in msg,
                unquote_msg=False,
            )
        ...
        hit = writer.wait_for_breakpoint_hit(
            REASON_CAUGHT_EXCEPTION,
            line=writer.get_line_index_with_content("handle on method1"),
        )
        check(hit)
...
def test_case_scapy(case_setup):
    with case_setup.test_file("_debugger_case_scapy.py") as writer:
        ...
        writer.reader_thread.set_messages_timeout(
            30
        )  # Starting scapy may be slow (timed out with 15 seconds on appveyor).
...
@pytest.mark.parametrize(
    "mixed_case", [True, False] if sys.platform == "win32" else [False]
)
def test_path_translation(case_setup, mixed_case):
    def get_file_in_client(writer):
        ...
        return os.path.join(
            os.path.dirname(file_in_client), "foo", "_debugger_case_path_translation.py"
        )
    ...
    def get_environ(writer):
        ...
        if mixed_case:
            new_file_in_client = "".join(
                [
                    file_in_client[i].upper()
                    if i % 2 == 0
                    else file_in_client[i].lower()
                    for i in range(len(file_in_client))
                ]
            )
            assert _path_equals(file_in_client, new_file_in_client)
        env["PATHS_FROM_ECLIPSE_TO_PYTHON"] = json.dumps(
            [(os.path.dirname(file_in_client), os.path.dirname(writer.TEST_FILE))]
        )
        return env
    ...
    with case_setup.test_file(
        ...
        writer.wait_for_message(
            lambda msg: "901\t" in msg
            and ("FileNotFoundError" in msg or "IOError" in msg),
            expect_xml=False,
        )
...
def test_show_bytecode_xml(case_setup, tmpdir):
    ...
    with case_setup.test_file("_debugger_case_show_bytecode.py") as writer:
        ...
        for frame_id in frame_ids:
            ...
            writer.wait_for_message(
                lambda msg: "%s\t" % CMD_LOAD_SOURCE_FROM_FRAME_ID in msg
                and ("MyClass" in msg or "foo()" in msg),
                expect_xml=False,
            )
...
def test_evaluate_errors(case_setup):
    with case_setup.test_file("_debugger_case_local_variables.py") as writer:
        ...
        writer.write_evaluate_expression(
            "%s\t%s\t%s" % (thread_id, frame_id, "LOCAL"), "name_error"
        )
...
def test_case_lamdda(case_setup):
    with case_setup.test_file("_debugger_case_lamda.py") as writer:
        writer.write_add_breakpoint(
            writer.get_line_index_with_content("Break here"), "None"
        )
        writer.write_make_initial_run()
...
@pytest.mark.skipif(
    IS_JYTHON, reason="Not working properly on Jython (needs investigation)."
)
def test_case_suspension_policy(case_setup):
    with case_setup.test_file("_debugger_case_suspend_policy.py") as writer:
        ...
        for i in range(3):
            ...
            hit = writer.wait_for_breakpoint_hit(
                (REASON_STOP_ON_BREAKPOINT, REASON_THREAD_SUSPEND)
            )
    ...
    with case_setup.test_file("_debugger_case_get_thread_stack.py") as writer:
        ...
        for request_thread_id in thread_id_to_name:
            ...
            if not files[0].endswith("_debugger_case_get_thread_stack.py"):
                raise AssertionError(
                    "Expected to find _debugger_case_get_thread_stack.py in files[0]. Found: %s"
                    % ("\n".join(files),)
                )

            if [filename for filename in files if filename.endswith("pydevd.py")]:
                raise AssertionError(
                    "Did not expect to find pydevd.py. Found: %s" % ("\n".join(files),)
                )
            if request_thread_id == hit.thread_id:
                assert (
                    len(msg.thread.frame) == 0
                )  # In main thread (must have no back frames).
                assert msg.thread.frame["name"] == "<module>"
            else:
                assert (
                    len(msg.thread.frame) > 1
                )  # Stopped in threading (must have back frames).
...
def test_case_dump_threads_to_stderr(case_setup):
    ...
    def is_stderr_ok(stderr):
        return (
            "Thread Dump" in stderr
            and "Thread pydevd.CommandThread  (daemon: True, pydevd thread: True)"
            in stderr
        )
...
def test_stop_on_start_regular(case_setup):
    with case_setup.test_file("_debugger_case_simple_calls.py") as writer:
        ...
        hit = writer.wait_for_breakpoint_hit(
            REASON_STOP_ON_START, file="_debugger_case_simple_calls.py", line=1
        )
...
def test_stop_on_start_m_switch(case_setup_m_switch):
    with case_setup_m_switch.test_file() as writer:
        ...
        hit = writer.wait_for_breakpoint_hit(
            REASON_STOP_ON_START, file="_debugger_case_m_switch.py", line=1
        )
...
def test_stop_on_start_entry_point(case_setup_m_switch_entry_point):
    with case_setup_m_switch_entry_point.test_file() as writer:
        ...
        hit = writer.wait_for_breakpoint_hit(
            REASON_STOP_ON_START, file="_debugger_case_module_entry_point.py", line=1
        )
...
def test_debug_zip_files(case_setup, tmpdir):
    ...
    zip_file.writestr(
        "zipped2/zipped_contents2.py", "def call_in_zip2():\n    return 1"
    )
    zip_file.close()

    with case_setup.test_file(
        "_debugger_case_zip_files.py", get_environ=get_environ
    ) as writer:
        writer.write_add_breakpoint(
            2,
            "None",
            filename=os.path.join(
                str(tmpdir.join("myzip.zip")), "zipped", "zipped_contents.py"
            ),
        )
        ...
        writer.write_add_breakpoint(
            ...
            "None",
            filename=os.path.join(
                str(tmpdir.join("myzip2.egg!")), "zipped2", "zipped_contents2.py"
            ),
        )
...
@pytest.mark.parametrize(
    "count", range(5)
)  # Call multiple times to exercise timing issues.
def test_multiprocessing_with_stopped_breakpoints(
    case_setup_multiprocessing, count, debugger_runner_simple
):
    import threading
    from tests_python.debugger_unittest import AbstractWriterThread

    with case_setup_multiprocessing.test_file(
        "_debugger_case_multiprocessing_stopped_threads.py"
    ) as writer:
        ...
        # each pid).
        assert (
            len(pydev_log.list_log_files(debugger_runner_simple.pydevd_debug_file)) == 2
        )
        writer.finished_ok = True
...
@pytest.mark.skipif(not IS_CPYTHON, reason="CPython only test.")
@pytest.mark.parametrize(
    "target", ["_debugger_case_quoting.py", "_debugger_case_subprocess_zip.py"]
)
...
@pytest.mark.skipif(
    not IS_CPYTHON or IS_MAC, reason="CPython only test (brittle on Mac)."
)
...
def test_attach_to_pid_halted(case_setup_remote):
    with case_setup_remote.test_file(
        "_debugger_case_attach_to_pid_multiple_threads.py", wait_for_port=False
    ) as writer:
        time.sleep(
            1
        )  # Give it some time to initialize and get to the proper halting condition
        _attach_to_writer_pid(writer)
...
def test_remote_debugger_threads(case_setup_remote):
    with case_setup_remote.test_file("_debugger_case_remote_threads.py") as writer:
        ...
        writer.write_change_variable(
            hit_in_thread1.thread_id, hit_in_thread1.frame_id, "wait", "False"
        )
        writer.wait_for_var('<xml><var name="" type="bool"')
        writer.write_change_variable(
            hit_in_thread2.thread_id, hit_in_thread2.frame_id, "wait", "False"
        )
        writer.wait_for_var('<xml><var name="" type="bool"')
...
def test_py_37_breakpoint_remote_no_import(case_setup_remote):
    def get_environ(writer):
        ...
        pydevd_dirname = os.path.join(
            os.path.dirname(writer.get_pydevd_file()), "pydev_sitecustomize"
        )
    ...
    with case_setup_remote.test_file(
        "_debugger_case_breakpoint_remote_no_import.py", get_environ=get_environ
    ) as writer:
        writer.write_make_initial_run()
...
def test_remote_debugger_multi_proc(case_setup_remote, authenticate):
    ...
    class _SecondaryMultiProcProcessWriterThread(
        debugger_unittest.AbstractWriterThread
    ):
        ...
        def run(self):
            ...
            if authenticate:
                self.wait_for_message(
                    lambda msg: "Client not authenticated." in msg, expect_xml=False
                )
                self.write_authenticate(
                    access_token=access_token, client_access_token=client_access_token
                )
                self.write_version()
    ...
    with case_setup_remote.test_file(
        ...
        writer.secondary_multi_proc_process_writer = (
            secondary_multi_proc_process_writer
        ...
        try:
            assert writer._sequence == 5 if not authenticate else 9, (
                "Found: %s" % writer._sequence
            )
        except:
            writer.log.append("assert failed!")
            raise
...
def test_trace_dispatch_correct(case_setup):
    def get_environ(writer):
        ...
        ] = "NO"  # This test checks trace dispatch (so, disable frame eval).
        return env

    with case_setup.test_file(
        "_debugger_case_trace_dispatch.py", get_environ=get_environ
    ) as writer:
...
@pytest.mark.skipif(
    IS_JYTHON, reason="Not working with Jython on ci (needs investigation)."
)
...
def test_gevent(case_setup):
    ...
    with case_setup.test_file(
        "_debugger_case_gevent.py", get_environ=get_environ
    ) as writer:
        writer.write_add_breakpoint(writer.get_line_index_with_content("break here"))
        writer.write_make_initial_run()
        for _i in range(10):
...
def test_gevent_remote(case_setup_remote):
    ...
    with case_setup_remote.test_file(
        ...
        get_environ=get_environ,
        append_command_line_args=["remote"],
    ) as writer:
        writer.write_add_breakpoint(writer.get_line_index_with_content("break here"))
        writer.write_make_initial_run()
        for _i in range(10):
...
def test_return_value(case_setup):
    with case_setup.test_file("_debugger_case_return_value.py") as writer:
        ...
        writer.write_step_over(hit.thread_id)
        hit = writer.wait_for_breakpoint_hit(
            REASON_STEP_OVER, name="main", line=break_line + 1
        )
        writer.write_get_frame(hit.thread_id, hit.frame_id)
        writer.wait_for_vars(
            [
                [
                    '<var name="method1" type="int" qualifier="%s" value="int: 1" isRetVal="True"'
                    % (builtin_qualifier,),
        ...
        writer.write_step_over(hit.thread_id)
        hit = writer.wait_for_breakpoint_hit(
            REASON_STEP_OVER, name="main", line=break_line + 2
        )
        writer.write_get_frame(hit.thread_id, hit.frame_id)
        writer.wait_for_vars(
            [
                [
                    '<var name="method2" type="int" qualifier="%s" value="int: 2" isRetVal="True"'
                    % (builtin_qualifier,),
...
def test_run_pause_all_threads_single_notification(
    case_setup, check_single_notification
):
    ...
    with case_setup.test_file("_debugger_case_multiple_threads.py") as writer:
        ...
        if check_single_notification:
            dct = writer.wait_for_json_message(
                CMD_THREAD_RESUME_SINGLE_NOTIFICATION
            )  # Note: prefer wait_for_single_notification_as_hit
        ...
        writer.write_evaluate_expression(
            "%s\t%s\t%s" % (hit.thread_id, hit.frame_id, "LOCAL"), "stop_loop()"
        )
        writer.wait_for_evaluation(
            '<var name="stop_loop()" type="str" qualifier="{0}" value="str: stopped_loop'.format(
                builtin_qualifier
            )
        )
...
def test_smart_step_into_case2(case_setup):
    with case_setup.test_file("_debugger_case_smart_step_into2.py") as writer:
        ...
        OFFSET_POS = 4
        writer.write_smart_step_into(
            hit.thread_id, "offset=" + found[2][OFFSET_POS], "foo"
        )
        hit = writer.wait_for_breakpoint_hit(reason=CMD_SMART_STEP_INTO)
        assert hit.line == writer.get_line_index_with_content("on foo mark")
...
def test_smart_step_into_case3(case_setup):
    with case_setup.test_file("_debugger_case_smart_step_into3.py") as writer:
        ...
        writer.write_smart_step_into(
            hit.thread_id,
            "offset=" + f[0][OFFSET_POS] + ";" + f[0][CHILD_OFFSET_POS],
            "foo",
        )
        hit = writer.wait_for_breakpoint_hit(reason=CMD_SMART_STEP_INTO)
        assert hit.line == writer.get_line_index_with_content("on foo mark")
...
def test_step_over_my_code(case_setup):
    with case_setup.test_file("my_code/my_code.py") as writer:
        ...
        hit = writer.wait_for_breakpoint_hit(
            reason=REASON_STEP_OVER_MY_CODE
        )  # Note: goes from step over to step into
        assert hit.name == "callback1"
...
def test_sysexit_on_filtered_file(case_setup):
    def get_environ(writer):
        env = os.environ.copy()
        env.update(
            {"PYDEVD_FILTERS": json.dumps({"**/_debugger_case_sysexit.py": True})}
        )
        return env

    with case_setup.test_file(
        "_debugger_case_sysexit.py", get_environ=get_environ, EXPECTED_RETURNCODE=1
    ) as writer:
        writer.write_add_exception_breakpoint_with_policy(
            "SystemExit",
            notify_on_handled_exceptions=1,  # Notify multiple times
...
def test_access_token(case_setup):
    ...
    with case_setup.test_file(
        "_debugger_case_print.py", update_command_line_args=update_command_line_args
    ) as writer:
        writer.write_add_breakpoint(
            1, "None"
        )  # I.e.: should not work (not authenticated).
    ...
    with case_setup.test_file("_debugger_case_namedtuple.py") as writer:
        ...
        for _ in range(2):
            ...
            hit = writer.wait_for_breakpoint_hit(
                ...
                file="_debugger_case_namedtuple.py",
                line=expected_line,
            )
...
def test_matplotlib_activation(case_setup):
    ...
    with case_setup.test_file(
        "_debugger_case_matplotlib.py", get_environ=get_environ
    ) as writer:
        writer.write_add_breakpoint(writer.get_line_index_with_content("break here"))
        writer.write_make_initial_run()
        for _ in range(3):
...
def test_stepin_not_my_code_coroutine(case_setup):
    ...
    with case_setup.test_file(
        "my_code/my_code_coroutine.py", get_environ=get_environ
    ) as writer:
        writer.write_set_project_roots(
            [debugger_unittest._get_debugger_test_file("my_code")]
        )
...
@pytest.mark.skipif(
    not IS_CPYTHON or not IS_PY36_OR_GREATER, reason="Only CPython 3.6 onwards"
)
...
def test_frame_eval_mode_corner_case_01(case_setup):
    with case_setup.test_file(
        ...
        hit = writer.wait_for_breakpoint_hit(
            line=writer.get_line_index_with_content("step 4"),
            file="_debugger_case_wrong_bytecode.py",
            reason=REASON_STEP_OVER,
        )
        writer.write_step_over(hit.thread_id)
        ...
        hit = writer.wait_for_breakpoint_hit(
            line=writer.get_line_index_with_content("step 5"),
            file="_debugger_case_wrong_bytecode.py",
            reason=REASON_STEP_OVER,
        )
        writer.write_run_thread(hit.thread_id)
...
def test_frame_eval_mode_corner_case_03(case_setup):
    with case_setup.test_file(
        ...
        writer.write_step_over(
            hit.thread_id
        )  # i.e.: check that the jump target is still ok.
        hit = writer.wait_for_breakpoint_hit(
            line=line, reason=REASON_STOP_ON_BREAKPOINT
        )
...
def test_debugger_shadowed_imports(case_setup, tmpdir, module_name_and_content):
    ...
    assert (
        'the module "%s" could not be imported because it is shadowed by:'
        % (module_name.split(".")[0])
    ) in writer.get_stderr()
