...
from _pydevd_bundle.pydevd_collect_bytecode_info import (
    ...
    collect_return_info,
    code_to_bytecode_representation,
)
...
def _method_simple_raise_unmatched_except():
    try:  # SETUP_EXCEPT (to except line)
        ...
        NameError
    ):  # DUP_TOP, LOAD_GLOBAL (NameError), COMPARE_OP (exception match), POP_JUMP_IF_FALSE
        pass
...
class _ExcVerifier(object):
    ...
    def check(
        ...
        try_except_infos = sorted(
            collect_try_except_info(code, use_func_first_line=True),
...
def test_collect_try_except_info3(exc_verifier):
    def method():
        ...
        try:  # SETUP_EXCEPT (to except line)
            raise AssertionError()
        except get_exc_class() as e:  # POP_TOP
            raise e
...
def test_collect_try_except_info_raise_unhandled7(exc_verifier):
    ...
    exc_verifier.check(
        raise_unhandled7, "[{try:1 except 3 end block 7}, {try:4 except 6 end block 7}]"
    )
...
def test_collect_try_except_info_return_on_except(exc_verifier):
    ...
    try_except_info_for_source = (
        "[{try:1 except 6 end block 10}, {try:2 except 4 end block 5 raises: 5}]"
    )
    if sys.version_info[:2] <= (3, 7):
        ...
        try_except_info_for_source = (
            "[{try:1 except 6 end block 9}, {try:2 except 4 end block 5 raises: 5}]"
        )
...
"""
    ...
    if IS_CPYTHON or IS_PYPY:
        ...
        from _pydevd_bundle.pydevd_collect_bytecode_info import (
            collect_try_except_info_from_contents,
        )

        assert (
            str(collect_try_except_info_from_contents(code_str))
            == "[{try:3 except 8 end block 9}]"
        )
...
def test_collect_return_info():
    ...
    assert (
        str(collect_return_info(method.__code__, use_func_first_line=True))
        == "[{return: 1}]"
    )
    ...
    assert (
        str(collect_return_info(method2.__code__, use_func_first_line=True))
        == "[{return: 1}]"
    )
    ...
    assert (
        str(collect_return_info(method3.__code__, use_func_first_line=True))
        == "[{return: 2}]"
    )
    ...
    assert (
        str(collect_return_info(method4.__code__, use_func_first_line=True))
        == "[{return: 1}]"
        if IS_PY38_OR_GREATER
        else "[{return: 4}]"
    )
    ...
    assert (
        str(collect_return_info(method5.__code__, use_func_first_line=True))
        == "[{return: 1}]"
        if IS_PY38_OR_GREATER
        else "[{return: 3}]"
    )
...
"""
    ...
    exec(code, scope)
    assert (
        str(collect_return_info(scope["method"].__code__, use_func_first_line=True))
        == "[{return: 4}, {return: 6}]"
    )
...
def test_simple_code_to_bytecode_repr():
    def method4():
        return (1, 2, 3, call("tnh %s" % 1))

    contents = code_to_bytecode_representation(
        method4.__code__, use_func_first_line=True
    )
...
def test_simple_code_to_bytecode_repr_many():
    ...
    new_repr = code_to_bytecode_representation(
        method4.__code__, use_func_first_line=True
    )
    # print(new_repr)
    ...
    for key, method in sorted(dict(globals()).items()):
        if key.startswith("_method"):
            ...
            if sys.version_info[0] < 3:
                from _pydevd_bundle.pydevd_collect_bytecode_info import (
                    _iter_as_bytecode_as_instructions_py2,
                )
...
def debug_test_tracing_output():  # Note: not run by default, only to debug tracing.
    ...
    for output, method_names in sorted(
        output_to_method_names.items(), key=lambda x: (-len(x[1]), "".join(x[1]))
    ):
        print("=" * 80)
