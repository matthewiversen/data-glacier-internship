...
from _pydev_bundle._pydev_completer import (
    isidentifier,
...
def test_extract_token_and_qualifier():
    ...
    assert extract_token_and_qualifier("tok.qual", 0, 5) == TokenAndQualifier(
        "tok", "q"
    )
    assert extract_token_and_qualifier("tok.qual", 0, 6) == TokenAndQualifier(
        "tok", "qu"
    )
    assert extract_token_and_qualifier("tok.qual", 0, 7) == TokenAndQualifier(
        "tok", "qua"
    )
    assert extract_token_and_qualifier("tok.qual", 0, 8) == TokenAndQualifier(
        "tok", "qual"
    )

    # out of range (column)
    assert extract_token_and_qualifier("tok.qual.qual2", 0, 100) == TokenAndQualifier(
        "tok.qual", "qual2"
    )
