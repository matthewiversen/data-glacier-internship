...
def test_create_frames_list_from_traceback():
    ...
    try:
        ...
        frames_list = create_frames_list_from_traceback(
            trace_obj,
            frame,
            exc_type,
            exc_desc,
        ...
        assert (
            str(frames_list.chained_frames_list.chained_frames_list.exc_desc) == "first"
        )
        assert (
            frames_list.chained_frames_list.chained_frames_list.chained_frames_list
            is None
        )
...
if IS_PY311_OR_GREATER:
    ...
    _byte_offset_to_character_offset = getattr(
        traceback, "_byte_offset_to_character_offset", None
    )
...
def test_collect_anchors_subscript(use_unicode):
    ...
    try:
        ...
        frame = None
        frames_list = create_frames_list_from_traceback(
            trace_obj, frame, exc_type, exc_desc, memo
        )
