...
pytestmark = pytest.mark.skipif(
    not IS_PY36_OR_GREATER or TODO_PY311 or not IS_CPYTHON or not TEST_CYTHON,
    reason="Requires CPython >= 3.6 < 3.11",
)
            ...
            if line in tracer.lines_executed:
                ...
                if stop_at_all_lines:
                    raise AssertionError(
                        "Expected the debugger to stop at all lines. Did not stop at line: %s"
                        % (line,)
                    )
...
def test_set_pydevd_break_01a():
    ...
    check(
        "_bytecode_overflow_example.py",
        _bytecode_overflow_example.check_backtrack,
        method_kwargs={"x": "f"},
    )
...
def test_set_pydevd_break_06(pyfile):
    ...
    check(
        "_bytecode_super.py",
        _bytecode_super.B,
        method_to_change=_bytecode_super.B.__init__,
        stop_at_all_lines=True,
    )
...
def test_set_pydevd_break_07():
    ...
    check(
        "_bytecode_overflow_example.py",
        _bytecode_overflow_example.offset_overflow,
        method_kwargs={"stream": StringIO()},
    )
...
def test_set_pydevd_break_08():
    ...
    check(
        "_bytecode_overflow_example.py",
        _bytecode_overflow_example.long_lines,
        stop_at_all_lines=True,
    )
