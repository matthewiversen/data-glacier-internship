...
from contourpy.util.mpl_util import (
    filled_to_mpl_paths,
    lines_to_mpl_paths,
    mpl_codes_to_offsets,
)
...
class MplDebugRenderer(MplRenderer):
    ...
    def _filled_to_lists_of_points_and_offsets(
        ...
        if fill_type == FillType.OuterCode:
            ...
            all_offsets = [
                mpl_codes_to_offsets(codes) for codes in filled[1] if codes is not None
            ]
    ...
    def filled(
        ...
        ax = self._get_ax(ax)
        all_points, all_offsets = self._filled_to_lists_of_points_and_offsets(
            filled, fill_type
        )
