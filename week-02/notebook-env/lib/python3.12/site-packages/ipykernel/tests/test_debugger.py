...
f(2, 3)"""
    ...
    while (
        msg.get("msg_type") != "debug_event" or msg["content"].get("event") != "stopped"
    ):
...
@pytest.mark.skipif(
    sys.version_info >= (3, 10), reason="TODO Does not work on Python 3.10"
)
...
my_test()"""
    ...
    # Get the scopes
    scopes = wait_for_debug_request(kernel_with_debug, "scopes", {"frameId": frame_id})[
        "body"
    ]["scopes"]
    ...
    globals_ = wait_for_debug_request(
        ...
        "variables",
        {
            "variablesReference": next(
                filter(lambda s: s["name"] == "Globals", scopes)
            )["variablesReference"]
        },
    )["body"]["variables"]
    ...
    assert (
        global_var["value"] == local_var["value"]
        and global_var["type"] == local_var["type"]
    )
